(function(){
  ...
  async function createReceiptPdf(data,settings,logoUrl,stampUrl){
    ...
    drawText(ctx,'RICEVUTA DI PAGAMENTO',tx,PAGE_MARGIN+89,{font:'24px Arial',color:primary,bold:true});
    drawText(ctx,`Documento collegato al preventivo ${data.quote_number||'—'}`,tx,PAGE_MARGIN+124,{font:'17px Arial',color:muted});
    ...
    const declaration=`Si attesta la ricezione del saldo relativo al preventivo n. ${data.quote_number||'—'} del ${dateIT(data.quote_date)}. L'importo complessivo del servizio risulta saldato.`;

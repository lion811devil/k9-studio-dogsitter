(function(){
  ...
  async function createServicePdf(data,kind,settings,logoUrl){
    ...
    if(data.title){ensure(110);page.y=drawNarrative(page.ctx,kind==='customer'?'DOCUMENTO CLIENTE':'DOCUMENTO DIPENDENTE',data.title,x,page.y,w,primary,lighten(primary,.95))+gap;}
    ensure(260);
    ...
    page.y=drawNarrative(page.ctx,kind==='customer'?'Riepilogo del servizio':'Rapporto del servizio',data.summary||data.report_text||data.operational_notes||'Nessun rapporto inserito.',x,page.y,w,primary,lighten(primary,.95))+gap;
    ...
    if(kind==='employee'&&data.incident_notes){ensure(190);page.y=drawNarrative(page.ctx,'Anomalie e note',data.incident_notes,x,page.y,w,'#c95f18','#fff3e9')+gap;}
    if(kind==='employee'&&data.operational_notes){ensure(190);page.y=drawNarrative(page.ctx,'Note interne',data.operational_notes,x,page.y,w,secondary,lighten(secondary,.96))+gap;}
    ensure(190);

(function(){
  ...
  async function createServicePdf(data,kind,settings,logoUrl){
    ...
    if(data.title){ensure(110);page.y=drawNarrative(page.ctx,kind==='customer'?'DOCUMENTO CLIENTE':'DOCUMENTO INTERNO',data.title,x,page.y,w,primary,lighten(primary,.95))+gap;}
    ensure(260);
  ...
  async function createReceiptPdf(data,settings,logoUrl,stampUrl){
    ...
    roundedRect(ctx,x,y,w,150,18,'#ffffff',line);roundedRect(ctx,x,y,10,150,6,secondary);drawText(ctx,'SERVIZIO',x+26,y+34,{font:'19px Arial',color:secondary,bold:true});
    label('Tipologia',x+26,y+66);value(data.service_type,x+26,y+92,w-52);
    label('Periodi da preventivo',x+26,y+122);drawText(ctx,pText||'—',x+215,y+122,{font:'18px Arial',color:ink,bold:true,maxWidth:w-240,lineHeight:21});y+=168;

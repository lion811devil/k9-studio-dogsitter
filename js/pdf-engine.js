(function(){
  ...
  async function createServicePdf(data,kind,settings,logoUrl){
    ...
    if(data.title){ensure(110);const sectionTitle=kind==='customer'?'DOCUMENTO CLIENTE':kind==='employee'?'DOCUMENTO DIPENDENTE':'DOCUMENTO INTERNO';page.y=drawNarrative(page.ctx,sectionTitle,data.title,x,page.y,w,primary,lighten(primary,.95))+gap;}
    ensure(260);
    ...
    const periodRows=periods.map((p,i)=>{const run=Array.isArray(data.period_runs)?data.period_runs.find(r=>Number(r.period_index)===i):null;const lifecycle=run?` · Stato ${String(run.status||'programmato').replaceAll('_',' ')}${run.started_at?' · iniziato '+dateTimeIT(run.started_at):''}${run.ended_at?' · terminato '+dateTimeIT(run.ended_at):''}`:'';return {label:`Periodo ${i+1}`,value:`${dateIT(p.start_date)}${p.end_date&&p.end_date!==p.start_date?' - '+dateIT(p.end_date):''} · ${Number(p.daily_visits||1)} uscite/giorno · ${[p.time_slot_1,p.time_slot_2,p.time_slot_3,p.time_slot_4].filter(Boolean).join(', ')||'orari da concordare'}${lifecycle}`}});
    ...
    ensure(sectionMetrics(page.ctx,serviceRows,w).height+gap);
    page.y=drawSection(page.ctx,'Servizio',serviceRows,x,page.y,w,secondary,lighten(secondary,.93))+gap;
    ensure(250);
    page.y=drawNarrative(page.ctx,kind==='customer'?'Riepilogo del servizio':kind==='employee'?'Rapporto per il dipendente':'Relazione amministrativa del servizio',data.summary||data.report_text||data.operational_notes||'Nessun rapporto inserito.',x,page.y,w,primary,lighten(primary,.95))+gap;
    if(kind==='customer'&&data.customer_observations){ensure(190);page.y=drawNarrative(page.ctx,'Osservazioni',data.customer_observations,x,page.y,w,secondary,lighten(secondary,.96))+gap;}
    if((kind==='employee'||kind==='internal')&&data.incident_notes){ensure(190);page.y=drawNarrative(page.ctx,'Anomalie e note',data.incident_notes,x,page.y,w,'#c95f18','#fff3e9')+gap;}
    if((kind==='employee'||kind==='internal')&&data.operational_notes){ensure(190);page.y=drawNarrative(page.ctx,kind==='internal'?'Note amministrative e operative':'Note per il dipendente',data.operational_notes,x,page.y,w,secondary,lighten(secondary,.96))+gap;}
    if(kind==='internal'){
      ensure(250);
      const customerTotal=Number(data.customer_amount||0),employeeTotal=Number(data.employee_compensation||0),margin=Number(data.internal_margin??(customerTotal-employeeTotal));
      roundedRect(page.ctx,x,page.y,w,220,24,lighten(primary,.94),primary);
      drawText(page.ctx,'QUADRO ECONOMICO INTERNO',x+34,page.y+46,{font:'25px Arial',color:darken(primary,.18),bold:true});
      const col=(w-68)/3;
      [['Totale cliente',money(customerTotal)],['Compenso dipendente',money(employeeTotal)],['Margine attività',money(margin)]].forEach((r,i)=>{
        const xx=x+34+i*col;
        drawText(page.ctx,r[0],xx,page.y+92,{font:'18px Arial',color:'#65747e',bold:true,maxWidth:col-18});
        drawText(page.ctx,r[1],xx,page.y+137,{font:'31px Arial',color:i===2?primary:secondary,bold:true,maxWidth:col-18});
      });
      drawText(page.ctx,`Cliente: ${data.customer_payment_status==='incassato'?'Incassato':'Da incassare'}  •  Dipendente: ${data.employee_payment_status==='liquidato'?'Liquidato':'Da liquidare'}`,x+34,page.y+190,{font:'20px Arial',color:'#43515a',bold:true,maxWidth:w-68});
      page.y+=244;
    }else{
      ensure(190);
      ...
      drawText(page.ctx,kind==='customer'?'TOTALE DOVUTO DAL CLIENTE':'COMPENSO DIPENDENTE',x+34,page.y+50,{font:'25px Arial',color:darken(econColor,.18),bold:true});
    ...
    if(kind==='customer'&&Number(data.deposit_amount||0)>0){ensure(220);page.y=drawSection(page.ctx,'Acconto e residuo',[{label:'Acconto ricevuto',value:money(data.deposit_amount)},{label:'Data ricezione',value:dateIT(data.deposit_received_at)},{label:'Modalità',value:data.deposit_payment_method||'—'},{label:'Residuo da pagare',value:money(data.balance_due??Math.max(0,Number(data.customer_amount||0)-Number(data.deposit_amount||0)))},{label:'Riferimento / nota',value:data.deposit_reference||'—'}],x,page.y,w,'#b56a12','#fff6e8')+gap;}
    if(settings.show_fiscal_data_pdf!==false && (kind==='customer'||kind==='internal')){
      ensure(190);
      const fiscal=[settings.vat_number&&`P.IVA ${settings.vat_number}`,settings.fiscal_code&&`C.F. ${settings.fiscal_code}`,settings.iban&&`IBAN ${settings.iban}`].filter(Boolean).join('  •  ');

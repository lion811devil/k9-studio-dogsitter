(function(){
  ...
  async function createReceiptPdf(data,settings,logoUrl,stampUrl){
    ...
    const pages=[newPage()];let page=pages[0];
    const {ctx}=page;
    if(settings.show_header_pdf!==false){
      roundedRect(ctx,PAGE_MARGIN,PAGE_MARGIN,A4.width-PAGE_MARGIN*2,245,28,primary);
      if(logo&&settings.show_logo_pdf!==false){const ratio=Math.min(180/logo.width,150/logo.height),w=logo.width*ratio,h=logo.height*ratio;roundedRect(ctx,PAGE_MARGIN+28,PAGE_MARGIN+34,210,176,20,'#ffffff');ctx.drawImage(logo,PAGE_MARGIN+43+(180-w)/2,PAGE_MARGIN+47+(150-h)/2,w,h)}
      const tx=logo&&settings.show_logo_pdf!==false?PAGE_MARGIN+270:PAGE_MARGIN+42;
      drawText(ctx,settings.organization_name||'K9 Napoletano Academy',tx,PAGE_MARGIN+62,{font:'34px Arial',color:'#ffffff',bold:true,maxWidth:690,lineHeight:40});
      if(settings.subtitle)drawText(ctx,settings.subtitle,tx,PAGE_MARGIN+96,{font:'18px Arial',color:'#e6f3f4',maxWidth:690,lineHeight:23});
      drawText(ctx,'RICEVUTA DI PAGAMENTO',tx,PAGE_MARGIN+132,{font:'27px Arial',color:lighten(primary,.82),bold:true});
      drawText(ctx,`Rif. ${data.receipt_number||'—'}`,tx,PAGE_MARGIN+178,{font:'21px Arial',color:'#ffffff'});
      roundedRect(ctx,A4.width-PAGE_MARGIN-246,PAGE_MARGIN+42,210,72,18,secondary);
      drawText(ctx,dateIT(data.receipt_date),A4.width-PAGE_MARGIN-220,PAGE_MARGIN+88,{font:'27px Arial',color:'#ffffff',bold:true});
      page.y=PAGE_MARGIN+280;
    }else page.y=PAGE_MARGIN;
    const x=PAGE_MARGIN,w=A4.width-PAGE_MARGIN*2,gap=24;
    page.y=drawSection(ctx,'Cliente',[{label:'Nome e cognome',value:data.customer_name||'—'},{label:'Animale',value:data.dog_name||'—'},{label:'Email',value:data.customer_email||''},{label:'Indirizzo',value:data.customer_address||''}],x,page.y,w,primary,lighten(primary,.93))+gap;
    const company=businessRows(settings);if(company.length)page.y=drawSection(ctx,'Dati attività',company,x,page.y,w,secondary,lighten(secondary,.96))+gap;
    ...
    const periodRows=periods.map((p,i)=>({label:`Periodo ${i+1}`,value:`${dateIT(p.start_date)}${p.end_date&&p.end_date!==p.start_date?' - '+dateIT(p.end_date):''} · ${Number(p.daily_visits||1)} uscite/giorno`}));
    page.y=drawSection(ctx,'Servizio',[{label:'Tipologia',value:data.service_type||'—'},...periodRows],x,page.y,w,secondary,lighten(secondary,.94))+gap;
    roundedRect(ctx,x,page.y,w,168,24,lighten(primary,.90),primary);
    drawText(ctx,'IMPORTO RICEVUTO',x+34,page.y+50,{font:'25px Arial',color:darken(primary,.18),bold:true});
    drawText(ctx,money(data.amount_received),x+34,page.y+114,{font:'48px Arial',color:primary,bold:true});
    drawText(ctx,`Modalità: ${data.payment_method||'—'}`,x+w-430,page.y+104,{font:'22px Arial',color:'#43515a',bold:true,maxWidth:380,lineHeight:27});
    page.y+=192;
    if(Number(data.deposit_amount||0)>0){page.y=drawSection(ctx,'Dettaglio pagamento',[{label:'Acconto già ricevuto',value:money(data.deposit_amount)},{label:'Saldo',value:money(Math.max(0,Number(data.amount_received||0)-Number(data.deposit_amount||0)))},{label:'Stato',value:'Pagato'}],x,page.y,w,'#b56a12','#fff6e8')+gap;}
    page.y=drawNarrative(ctx,'Dichiarazione',`Si attesta la ricezione dell'importo sopra indicato a saldo del servizio descritto nel presente documento.`,x,page.y,w,primary,lighten(primary,.97))+gap;
    if(settings.show_fiscal_data_pdf!==false){const fiscal=[settings.vat_number&&`P.IVA ${settings.vat_number}`,settings.fiscal_code&&`C.F. ${settings.fiscal_code}`].filter(Boolean).join('  •  ');if(fiscal)page.y=drawNarrative(ctx,'Dati amministrativi',fiscal,x,page.y,w,secondary,lighten(secondary,.96))+gap;}
    // Timbro + firma: 57 x 22 mm esatti sulla pagina A4 (1240 px / 210 mm).
    const stampW=A4.width*(57/210),stampH=A4.width*(22/210);
    const stampX=A4.width-PAGE_MARGIN-stampW,stampY=Math.min(page.y+18,A4.height-300);
    drawText(ctx,'Timbro e firma',stampX,stampY-12,{font:'18px Arial',color:'#6f7f88',bold:true});
    if(stamp)ctx.drawImage(stamp,stampX,stampY,stampW,stampH);
    page.y=stampY+stampH+30;
    const total=pages.length;pages.forEach((pg,i)=>drawFooter(pg,settings,i+1,total));
    const jpgs=[];for(const pg of pages)jpgs.push(await canvasToJpeg(pg.canvas));return buildImagePdf(jpgs);
  }

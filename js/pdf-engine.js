(function(){
  ...
  function drawFooter(page,settings,pageNo,total){
    const {ctx}=page;
    const y=A4.height-72;
    ctx.strokeStyle='#dce5ea';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(PAGE_MARGIN,y-28);ctx.lineTo(A4.width-PAGE_MARGIN,y-28);ctx.stroke();
    drawText(ctx,settings.footer_text||settings.legal_text||settings.organization_name||'',PAGE_MARGIN,y,{font:'17px Arial',color:'#6f7f88',maxWidth:870,lineHeight:22});
    drawText(ctx,`Pagina ${pageNo} di ${total}`,A4.width-PAGE_MARGIN-120,y,{font:'17px Arial',color:'#6f7f88'});
  }
  ...
  async function createServicePdf(data,kind,settings,logoUrl){
    ...
    const ensure=(height)=>{
      if(page.y+height>A4.height-150){page=newPage();pages.push(page);drawHeader(page,data,kind,settings,logo);}
    };
    ensure(260);
  ...
  async function createQuotePdf(input,settings,logoUrl){
    ...
    const drawQuoteHeader=()=>{
      const {ctx}=page;roundedRect(ctx,PAGE_MARGIN,PAGE_MARGIN,A4.width-PAGE_MARGIN*2,245,28,primary);
    ...
    const ensure=height=>{if(page.y+height>A4.height-150){page=newPage();pages.push(page);drawQuoteHeader()}};
    ensure(260);page.y=drawSection(page.ctx,'Cliente',[{label:'Nome e cognome',value:data.customer_name||'—'},{label:'Cane',value:data.dog_name||'—'},{label:'Telefono',value:data.customer_phone||''},{label:'Email',value:data.customer_email||''},{label:'Indirizzo',value:data.customer_address||''}],x,page.y,w,primary,lighten(primary,.93))+gap;
    const quotePeriods=Array.isArray(data.periods)&&data.periods.length?data.periods:[];
    if(quotePeriods.length){ensure(150+quotePeriods.length*58);const rows=quotePeriods.map((p,i)=>({label:`Periodo ${i+1}`,value:`${dateIT(p.start_date)}${p.end_date&&p.end_date!==p.start_date?' - '+dateIT(p.end_date):''} · ${Number(p.daily_visits||1)} uscite/giorno · ${[p.time_slot_1,p.time_slot_2,p.time_slot_3,p.time_slot_4].filter(Boolean).join(', ')||'orari da concordare'}`}));page.y=drawSection(page.ctx,'Periodi del servizio',rows,x,page.y,w,secondary,lighten(secondary,.95))+gap;}

...
function renderStatistics(){
  const host=$('#statisticsPanel');if(!host)return;const r=statsRange(),services=state.services.filter(s=>s.status!=='annullato'&&recordInRange(s,r.from,r.to));const maturedRows=services.filter(opIsMatured);const revenue=maturedRows.reduce((a,s)=>a+proratedServiceAmount(s,r.from,r.to,'customer'),0),comp=maturedRows.reduce((a,s)=>a+proratedServiceAmount(s,r.from,r.to,'employee'),0),margin=revenue-comp,visits=services.reduce((a,s)=>a+visitsInRange(s,r.from,r.to),0);
  const base=new Date(),datedServices=state.services.map(s=>parseLocalDate(serviceFirstDate(s))).filter(Boolean),earliest=datedServices.length?new Date(Math.min(...datedServices.map(d=>d.getTime()))):base,allMonths=Math.max(1,(base.getFullYear()-earliest.getFullYear())*12+(base.getMonth()-earliest.getMonth())+1),monthCount=operationsState.statisticsPeriod==='year'?12:(operationsState.statisticsPeriod==='all'?allMonths:1);const byMonth=Array.from({length:monthCount},(_,i)=>{const d=operationsState.statisticsPeriod==='year'?new Date(base.getFullYear(),i,1):new Date(base.getFullYear(),base.getMonth()-(monthCount-1-i),1);const from=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-01`,to=localDate(new Date(d.getFullYear(),d.getMonth()+1,0)),rows=state.services.filter(s=>s.status!=='annullato'&&recordInRange(s,from,to));return {label:d.toLocaleDateString('it-IT',{month:'short',year:operationsState.statisticsPeriod==='all'?'2-digit':undefined}),services:rows.length,revenue:rows.filter(opIsMatured).reduce((a,s)=>a+proratedServiceAmount(s,from,to,'customer'),0)}});

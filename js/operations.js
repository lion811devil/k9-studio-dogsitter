...
function renderOperationalDashboard(){
  ...
  const upcoming=state.services.filter(s=>opOperationalService(s)&&normalizePeriods(s).some(p=>(p.end_date||p.start_date)>=today)).sort((a,b)=>String(opFirstPeriodStart(a)+(a.service_time||'')).localeCompare(String(opFirstPeriodStart(b)+(b.service_time||'')))).slice(0,6);

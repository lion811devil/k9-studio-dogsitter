...
window.deleteAppNotification=async id=>{const n=state.notifications.find(x=>x.id===id);if(!n)return toast('Notifica non trovata');await archiveEntity('app_notifications',id,`notifica ${n.title||''}`.trim());knownNotificationIds.delete(id);buildNav();renderNotifications()};
...
async function loadDocuments(){
  ...
  try{serviceDocs=await select('dogsitter_document_versions','select=*&deleted_at=is.null&order=created_at.desc')}catch(e){console.warn('Archivio servizi versionato non ancora installato:',e.message);try{serviceDocs=await select('dogsitter_documents','select=*&order=created_at.desc')}catch{serviceDocs=[]}}
  try{quoteDocs=(await select('dogsitter_quote_document_versions','select=*&deleted_at=is.null&order=created_at.desc')).map(d=>({...d,source_kind:'quote',document_type:'quote'}))}catch(e){console.warn('Archivio preventivi non ancora installato:',e.message)}
  try{receiptDocs=(await select('dogsitter_receipts','select=*&deleted_at=is.null&order=created_at.desc')).map(d=>({...d,source_kind:'receipt',document_type:'receipt',version:1,is_active:true}))}catch(e){console.warn('Archivio ricevute non ancora installato:',e.message)}
...
async function loadAll(){if(isAdmin()){[state.customers,state.dogs,state.profiles,state.services,state.quotes,state.documents,state.communications,state.communicationReads,state.notifications,state.periodRuns,state.pdfDrafts]=await Promise.all([select('customers','select=*&deleted_at=is.null&order=last_name.asc,first_name.asc'),select('dogs','select=*&deleted_at=is.null&order=name.asc'),select('profiles','select=*&deleted_at=is.null&order=full_name.asc'),select('dogsitter_services','select=*&deleted_at=is.null&order=service_date.desc,service_time.asc'),loadQuotes(),loadDocuments(),select('service_communications','select=*&order=created_at.asc'),loadCommunicationReads(),select('app_notifications','select=*&deleted_at=is.null&order=created_at.desc'),select('service_period_workflows','select=*&order=service_id.asc,period_index.asc'),select('service_pdf_drafts','select=*&order=updated_at.desc')]);state.compensationServices=state.services;state.compensationPeriodRuns=state.periodRuns}else{const d=await rpc('employee_workspace');state.customers=d.customers||[];state.dogs=d.dogs||[];state.compensationServices=d.compensation_services||[];state.compensationPeriodRuns=d.compensation_period_runs||[];const operational=(d.services||[]).filter(x=>x&&!x.deleted_at&&['programmato','in_corso','da_verificare'].includes(String(x.status||'')));const operationalIds=new Set(operational.map(x=>x.id));const recovered=(state.compensationServices||[]).filter(x=>x&&!x.deleted_at&&!operationalIds.has(x.id)&&['programmato','in_corso','da_verificare'].includes(String(x.status||'')));state.services=[...operational,...recovered].sort((a,b)=>String(b.service_date||'').localeCompare(String(a.service_date||''))||String(a.service_time||'').localeCompare(String(b.service_time||'')));state.periodRuns=(d.period_runs||[]).filter(r=>r&&state.services.some(s=>s.id===r.service_id));state.quotes=[];state.documents=[];state.profiles=[];state.pdfDrafts=[];if(recovered.length)console.warn('Workspace dipendente: recuperati solo servizi con stato operativo coerente:',recovered.map(x=>x.id));[state.communications,state.communicationReads,state.notifications]=await Promise.all([select('service_communications','select=*&order=created_at.asc'),loadCommunicationReads(),select('app_notifications','select=*&order=created_at.desc')])}}
...
function renderDashboard(){
  ...
  const maturedToday=compensationPeriodRows().filter(x=>x.period.start_date<=today&&(x.period.end_date||x.period.start_date)>=today).reduce((a,x)=>a+x.amount,0);
  $('#dashboard').innerHTML=`<div class="page-title"><div><span class="eyebrow">AREA DIPENDENTE</span><h2>Il mio lavoro di oggi</h2></div></div><div class="stats dashboard-stats"><div class="stat stat-today"><span>Servizi</span><strong>${todays.length}</strong><small>Assegnati oggi</small></div><div class="stat stat-dogs"><span>Uscite</span><strong>${todays.reduce((a,s)=>a+visitsOnDate(s,today),0)}</strong><small>Totale previsto</small></div><div class="stat stat-pay"><span>Compenso maturato</span><strong>${money(maturedToday)}</strong><small>Solo periodi verificati</small></div></div><div class="grid service-grid">${todays.map(serviceCard).join('')||'<div class="card empty-state"><strong>Nessun servizio oggi</strong><span>Non risultano attività assegnate.</span></div>'}</div>`
}
...
function dogDetailRow(label,value){
  const text=(value===undefined||value===null||String(value).trim()==='')?'Non indicato':String(value).trim();
  return `<p class="wide"><span>${esc(label)}</span><strong>${esc(text)}</strong></p>`;
}
function dogDetailSection(title,rows){
  return `<div class="wide" style="grid-column:1/-1;margin-top:8px"><span style="display:block;font-weight:800;letter-spacing:.04em;text-transform:uppercase;margin-bottom:4px">${esc(title)}</span></div>${rows.join('')}`;
}
function renderCustomers(){
  ...
  $('#customerList').innerHTML=state.customers.map(c=>{
    ...
    const rows=[
      dogDetailSection('Dati principali',[
        dogDetailRow('Nome',c.first_name),
        dogDetailRow('Cognome',c.last_name),
        dogDetailRow('Telefono',firstValue(c,'phone')),
        dogDetailRow('Email',firstValue(c,'email')),
        dogDetailRow('Indirizzo',firstValue(c,'address')),
        dogDetailRow('CAP',firstValue(c,'postal_code')),
        dogDetailRow('Comune',firstValue(c,'city')),
        ...(isAdmin()?[dogDetailRow('Dipendente assegnato',c.assigned_employee_id?pname(c.assigned_employee_id):'Non assegnato')]:[])
      ]),
      dogDetailSection('Emergenza e reperibilità',[
        dogDetailRow('Tipo contatto di emergenza',firstValue(c,'emergency_contact_type')),
        dogDetailRow('Nome contatto di emergenza',firstValue(c,'emergency_contact_name')),
        dogDetailRow('Telefono contatto di emergenza',firstValue(c,'emergency_contact_phone')),
        dogDetailRow('Disponibilità / reperibilità',firstValue(c,'availability'))
      ]),
      dogDetailSection('Note operative',[
        dogDetailRow('Note operative',firstValue(c,'operational_notes'))
      ]),
      dogDetailSection('Collegamenti',[
        dogDetailRow('Animali associati',dogs.length?dogs.map(d=>d.name).join(', '):'Nessuno'),
        ...(next?[dogDetailRow('Data prossimo servizio',servicePeriod(next)),dogDetailRow('Fasce orarie prossimo servizio',serviceSlots(next)),dogDetailRow('Tipo prossimo servizio',next.service_type||'Servizio')]:[])
      ])
    ].join('');
    return `<details class="entity-accordion customer-accordion"><summary><span class="accordion-name">${esc(c.first_name)} ${esc(c.last_name)}</span><span class="accordion-chevron" aria-hidden="true"></span></summary><article class="entity-expanded"><div class="entity-card-head"><div class="entity-avatar">${esc((c.first_name||'?').slice(0,1)+(c.last_name||'').slice(0,1))}</div><div><span class="entity-kicker">CLIENTE</span><h3>${esc(c.first_name)} ${esc(c.last_name)}</h3></div></div><div class="entity-details">${rows}</div>${isAdmin()?customerCrmSections(c,dogs,services):''}${isAdmin()?`<div class="card-actions"><button class="primary-soft" onclick="openCustomer('${c.id}')">Apri / modifica</button><button onclick="openQuote(null,'${c.id}')">Crea preventivo</button><button onclick="openService(null,{customer_id:'${c.id}'})">Nuovo servizio</button><button class="danger" onclick="archiveEntity('customers','${c.id}','cliente ${esc(c.first_name+' '+c.last_name)}')">Elimina</button></div>`:''}</article></details>`
...
function renderDogs(){
  ...
  $('#dogList').innerHTML=state.dogs.map(d=>{
    ...
    const rows=[
      dogDetailSection('Identificazione',[
        dogDetailRow('Proprietario',cname(d.customer_id)),
        dogDetailRow('Tipo animale',firstValue(d,'animal_type')),
        dogDetailRow('Razza / tipologia',firstValue(d,'breed_type')),
        dogDetailRow('Dettaglio razza / tipologia',firstValue(d,'breed_detail','breed')),
        dogDetailRow('Taglia',firstValue(d,'size')),
        dogDetailRow('Fascia età',firstValue(d,'age_range')),
        dogDetailRow('Età precisa',firstValue(d,'age_detail')),
        dogDetailRow('Peso indicativo',firstValue(d,'weight_detail')),
        dogDetailRow('Sesso',firstValue(d,'sex')),
        dogDetailRow('Sterilizzato / castrato',firstValue(d,'sterilized')),
        dogDetailRow('Microchip',firstValue(d,'microchip_status')),
        dogDetailRow('Numero microchip',firstValue(d,'microchip_number')),
        dogDetailRow('Veterinario',firstValue(d,'vet_status')),
        dogDetailRow('Nome veterinario / clinica',firstValue(d,'vet_name')),
        dogDetailRow('Telefono veterinario / clinica',firstValue(d,'vet_phone'))
      ]),
      dogDetailSection('Salute e sicurezza',[
        dogDetailRow('Vaccinazioni',firstValue(d,'vaccines')),
        dogDetailRow('Antiparassitario',firstValue(d,'parasites')),
        dogDetailRow('Patologie note',firstValue(d,'illnesses')),
        dogDetailRow('Dettaglio patologie',firstValue(d,'illnesses_detail')),
        dogDetailRow('Allergie',firstValue(d,'allergies')),
        dogDetailRow('Dettaglio allergie',firstValue(d,'allergies_detail')),
        dogDetailRow('Farmaci / terapie',firstValue(d,'medicines')),
        dogDetailRow('Dettaglio farmaci / dosaggio',firstValue(d,'medicines_detail')),
        dogDetailRow('Livello rischio sanitario',firstValue(d,'health_risk'))
      ]),
      dogDetailSection('Alimentazione e routine',[
        dogDetailRow('Tipo alimentazione',firstValue(d,'food_type')),
        dogDetailRow('Marca / alimento / note dieta',firstValue(d,'food_detail')),
        dogDetailRow('Numero pasti',firstValue(d,'meals')),
        dogDetailRow('Orari pasti',firstValue(d,'meal_times')),
        dogDetailRow('Premi alimentari',firstValue(d,'treats')),
        dogDetailRow('Premi consentiti / vietati',firstValue(d,'treats_detail')),
        dogDetailRow('Regole domestiche',firstValue(d,'home_rules')),
        dogDetailRow('Dettaglio regole domestiche',firstValue(d,'home_rules_detail')),
        dogDetailRow('Routine quotidiana',firstValue(d,'routine_notes'))
      ]),
      dogDetailSection('Comportamento',[
        dogDetailRow('Carattere generale',firstValue(d,'character')),
        dogDetailRow('Con adulti',firstValue(d,'adults')),
        dogDetailRow('Con bambini',firstValue(d,'children')),
        dogDetailRow('Con altri cani',firstValue(d,'dogs_social')),
        dogDetailRow('Paure / fobie',firstValue(d,'fears')),
        dogDetailRow('Dettaglio paure / fobie',firstValue(d,'fears_detail')),
        dogDetailRow('Storico morsi',firstValue(d,'bite_history')),
        dogDetailRow('Dettaglio episodio morso / pinzata',firstValue(d,'bite_history_detail')),
        dogDetailRow('Possessività',firstValue(d,'resource_guarding')),
        dogDetailRow('Dettaglio possessività',firstValue(d,'resource_guarding_detail')),
        dogDetailRow('Altre note comportamentali',firstValue(d,'behavior_notes'))
      ]),
      dogDetailSection('Passeggiata',[
        dogDetailRow('Attrezzatura usata',firstValue(d,'equipment')),
        dogDetailRow('Dettaglio attrezzatura / taglia',firstValue(d,'equipment_detail')),
        dogDetailRow('Reazione ad altri cani',firstValue(d,'dog_triggers')),
        dogDetailRow('Reazione a bici / auto / runner',firstValue(d,'moving_triggers')),
        dogDetailRow('Zone da evitare',firstValue(d,'avoid_areas')),
        dogDetailRow('Dettaglio zone da evitare',firstValue(d,'avoid_areas_detail')),
        dogDetailRow('Libero dal guinzaglio',firstValue(d,'off_leash')),
        dogDetailRow('Livello gestione passeggiata',firstValue(d,'walk_level'))
      ])
    ].join('');
    return `<details class="entity-accordion dog-accordion"><summary><span class="accordion-name">${esc(d.name)}</span><span class="accordion-chevron" aria-hidden="true"></span></summary><article class="entity-expanded"><div class="entity-card-head"><div class="entity-avatar dog-avatar">🐾</div><div><span class="entity-kicker">${esc((d.animal_type||'Animale').toUpperCase())}</span><h3>${esc(d.name)}</h3><p class="entity-subtitle">${esc(breed)}</p></div></div><div class="entity-details">${rows}</div>${linkedDocumentsBlock({dogId:d.id},'PDF associati all’animale')}${isAdmin()?`<div class="card-actions"><button class="primary-soft" onclick="openDog('${d.id}')">Apri / modifica</button><button class="danger" onclick="archiveEntity('dogs','${d.id}','animale ${esc(d.name)}')">Elimina</button></div>`:''}</article></details>`
...
function renderEmployees(){
  ...
  $('#employeeList').innerHTML=groups.map(group=>`<details class="account-group" open><summary><span>${group.label}</span><strong>${group.rows.length}</strong></summary><div class="account-group-body">${group.rows.map(p=>{
    const assigned=state.services.filter(s=>s.employee_id===p.id&&s.status!=='annullato').length;
    const details=[
      dogDetailRow('Nome completo',p.full_name||p.email),
      dogDetailRow('Email',p.email),
      dogDetailRow('Codice',p.employee_code||'Non assegnato'),
      dogDetailRow('Qualifica',p.qualification||'Non indicata'),
      dogDetailRow('Ruolo',roleLabels[p.role]||p.role),
      dogDetailRow('Stato',p.active?'Attivo':'Sospeso'),
      dogDetailRow('Scadenza pass',p.pass_expires_at?dateIT(p.pass_expires_at):'Non indicata'),
      dogDetailRow('Servizi associati',assigned)
    ].join('');
    return `<details class="account-row" data-profile-id="${p.id}"><summary><div class="account-summary-main"><span class="entity-avatar employee-avatar">${esc((p.full_name||p.email||'?').slice(0,2).toUpperCase())}</span><div><strong>${esc(p.full_name||p.email)}</strong><small>${esc(roleLabels[p.role]||p.role)} · ${p.active?'Attivo':'Sospeso'}</small></div></div><span class="account-chevron">⌄</span></summary><div class="account-expanded"><div class="entity-details">${details}</div>${p.is_owner?'<div class="protected-note">Titolare protetto</div>':''}<div class="card-actions"><button class="primary-soft" onclick="openEmployee('${p.id}')">Seleziona e gestisci</button>${!p.is_owner&&p.active?`<button onclick="deactivateUser('${p.id}','${esc(p.full_name||p.email)}')">Disattiva</button>`:''}${!p.is_owner&&!p.active?`<button onclick="reactivateUser('${p.id}','${esc(p.full_name||p.email)}')">Riattiva</button>`:''}${!p.is_owner?`<button class="danger" onclick="archiveEntity('profiles','${p.id}','account ${esc(p.full_name||p.email)}')">Elimina</button>`:''}</div></div></details>`
  }).join('')||'<div class="empty-compact">Nessun account in questa categoria.</div>'}</div></details>`).join('')
}
...
function servicePeriodWorkflowBlock(s){
  ...
  return `<div class="period-workflow"><strong>Gestione periodi</strong>${periods.map((p,i)=>{
    ...
    const canVerify=admin&&s.status==='da_verificare'&&r.status==='da_verificare';
    ...
    if(closed){
      ...
      const settlementAction=!paid
        ...
        : allReady
          ...
          : '';
      actions=`<button class="primary-soft" onclick="viewService('${s.id}')">Apri</button><button onclick="addAssignedServiceToCalendar('${s.id}')">Collega a calendario</button><button class="communication-button" onclick="openServiceCommunications('${s.id}')">Comunicazioni${unreadCommunicationCount(s.id)?` <span>${unreadCommunicationCount(s.id)}</span>`:''}</button><button onclick="openService('${s.id}')">Modifica</button>${settlementAction}${s.status==='da_verificare'?`<button onclick="openPdfReviewHub('${s.id}')">Documenti e chiusura</button>`:''}<button class="danger" onclick="archiveEntity('dogsitter_services','${s.id}','servizio ${esc(s.service_type||'')} di ${esc(cname(s.customer_id))}')">Elimina</button>`;
    }
  }
...
function renderQuotes(){
  ...
  host.innerHTML=state.quotes.map(q=>{
    ...
    const periodRows=periods.map((p,i)=>[
      dogDetailRow(`Periodo ${i+1} · Data inizio`,dateIT(p.start_date)),
      dogDetailRow(`Periodo ${i+1} · Data fine`,dateIT(p.end_date||p.start_date)),
      dogDetailRow(`Periodo ${i+1} · Prestazioni al giorno`,p.daily_visits),
      dogDetailRow(`Periodo ${i+1} · Fascia oraria 1`,p.time_slot_1),
      dogDetailRow(`Periodo ${i+1} · Fascia oraria 2`,p.time_slot_2),
      dogDetailRow(`Periodo ${i+1} · Fascia oraria 3`,p.time_slot_3),
      dogDetailRow(`Periodo ${i+1} · Fascia oraria 4`,p.time_slot_4),
      dogDetailRow(`Periodo ${i+1} · Prestazioni totali`,periodVisits(p))
    ].join('')).join('');
    const itemRows=items.map((i,n)=>[
      dogDetailRow(`Voce ${n+1} · Descrizione`,i.description),
      dogDetailRow(`Voce ${n+1} · Quantità`,i.quantity),
      dogDetailRow(`Voce ${n+1} · Prezzo unitario`,money(i.unit_price)),
      dogDetailRow(`Voce ${n+1} · Totale`,money(Number(i.quantity||0)*Number(i.unit_price||0)))
    ].join('')).join('');
    const details=[
      dogDetailSection('Dati preventivo',[
        dogDetailRow('Cliente',cname(q.customer_id)),
        dogDetailRow('Animale',dname(q.dog_id)),
        dogDetailRow('Data preventivo',dateIT(q.quote_date)),
        dogDetailRow('Valido fino al',dateIT(q.valid_until)),
        dogDetailRow('Stato preventivo',quoteStatusLabels[q.status]||q.status),
        dogDetailRow('Stato cliente',q.client_status),
        dogDetailRow('Frequenza',q.frequency),
        dogDetailRow('Durata prevista',q.planned_duration_minutes!==undefined&&q.planned_duration_minutes!==null?`${Number(q.planned_duration_minutes)} minuti`:null)
      ]),
      dogDetailSection('Periodi e orari',[periodRows||dogDetailRow('Periodi','Non indicati')]),
      dogDetailSection('Voci economiche',[itemRows||dogDetailRow('Voci','Nessuna voce')]),
      dogDetailSection('Pagamento',[
        dogDetailRow('Subtotale',money(q.subtotal_amount||0)),
        dogDetailRow('Sconto',`${Number(q.discount_rate||0)}%`),
        dogDetailRow('Totale',money(quoteTotal(q))),
        dogDetailRow('Modalità di pagamento',q.payment_terms),
        dogDetailRow('Stato pagamento',q.payment_status),
        dogDetailRow('Acconto ricevuto',Number(q.deposit_amount||0)>0?money(q.deposit_amount):'Nessun acconto'),
        dogDetailRow('Data acconto',q.deposit_received_at?dateIT(q.deposit_received_at):null),
        dogDetailRow('Modalità acconto',q.deposit_payment_method),
        dogDetailRow('Riferimento / CRO',q.deposit_reference),
        dogDetailRow('Residuo',money(q.balance_due??Math.max(0,quoteTotal(q)-Number(q.deposit_amount||0))))
      ]),
      dogDetailSection('Organizzazione e autorizzazioni',[
        dogDetailRow('Chiavi affidate',q.keys_status),
        dogDetailRow('Consegna chiavi',q.keys_mode),
        dogDetailRow('Aggiornamenti al proprietario',q.customer_updates),
        dogDetailRow('Contatto veterinario in emergenza',q.auth_vet),
        dogDetailRow('Trasporto in clinica veterinaria',q.auth_transport),
        dogDetailRow('Note',q.notes)
      ])
    ].join('');
    return `<details class="record-accordion quote-accordion"><summary><div class="record-summary-main"><span class="record-title">${esc(cname(q.customer_id))}</span><small>${esc(dname(q.dog_id))} · ${dateIT(q.quote_date)} · ${esc(itemSummary||'Preventivo')}</small></div><strong class="record-amount">${money(quoteTotal(q))}</strong><span class="status-badge status-${esc(q.status)}">${esc(quoteStatusLabels[q.status]||q.status)}</span><span class="accordion-chevron" aria-hidden="true"></span></summary><article class="record-expanded quote-card"><div class="entity-details">${details}</div><div class="service-actions"><button class="primary-soft" onclick="openQuote('${q.id}')">Modifica</button><button class="primary" onclick="generateQuoteDocument('${q.id}')">Genera PDF</button><button onclick="setQuoteStatus('${q.id}','inviato')">Segna inviato</button><button onclick="setQuoteStatus('${q.id}','accettato')">Accettato</button><button onclick="setQuoteStatus('${q.id}','rifiutato')">Rifiutato</button><button onclick="transformQuoteToService('${q.id}')">Trasforma in servizio</button><button class="danger" onclick="archiveEntity('dogsitter_quotes','${q.id}','preventivo di ${esc(cname(q.customer_id))}')">Elimina</button></div></article></details>`
...
function compensationPeriodRows({includeUnmatured=false}={}){
  ...
  return runSource.map(r=>{
    ...
    if(['chiuso','annullato'].includes(String(service.status||''))&&!maturedRun)return null;
...
function closedServiceArchiveCard(s){
  ...
  const one=(label,d,kind)=>d
    ? `<div class="archive-doc-row"><b>${label}</b><button onclick="downloadStoredDocument('${d.id}')">Scarica</button><button onclick="downloadClosedServiceDocument('${s.id}','${kind}',true)">Nuova versione</button><button class="danger" onclick="deleteDocumentPermanently('${d.id}')">Elimina</button></div>`
...
window.openCustomer=id=>{
  ...
  const html=`<div class="data-form-stack">
    ${formSection('Dati principali','Anagrafica, contatti e indirizzo',`${field('first_name','Nome',c.first_name,'text','required')}${field('last_name','Cognome',c.last_name,'text','required')}${field('phone','Telefono',c.phone,'tel')}${field('email','Email',c.email,'email')}${field('address','Indirizzo',c.address)}${field('postal_code','CAP',c.postal_code)}${field('city','Comune',c.city)}<label>Dipendente predefinito per nuovi servizi<select name="assigned_employee_id"><option value="">Non assegnato</option>${em.map(p=>`<option value="${p.id}" ${p.id===c.assigned_employee_id?'selected':''}>${esc(p.full_name)}</option>`).join('')}</select><small class="muted">L’assegnazione operativa è sempre registrata nel singolo servizio.</small></label>`,true)}
...
window.openService=(id,seed={})=>{
  ...
  <section><h3>Assegnazione</h3><div class="form-grid">
    ...
    <label>Dipendente responsabile del servizio<select id="serviceEmployee" name="employee_id" required><option value="">Seleziona dipendente</option>${employees.map(p=>`<option value="${p.id}" ${p.id===assignedEmployee?'selected':''}>${esc(p.full_name)}</option>`).join('')}</select><small class="muted">Solo questo dipendente vedrà il servizio nella propria area.</small></label>
  ...
  <section><h3>Organizzazione e autorizzazioni</h3><div class="form-grid">
    ...
    if(!payload.customer_id||!payload.dog_id||!payload.employee_id)throw Error('Seleziona cliente, animale e dipendente.');const assignedProfile=state.profiles.find(p=>p.id===payload.employee_id&&p.role==='dipendente'&&p.active);if(!assignedProfile)throw Error('Il dipendente selezionato non è disponibile o non è attivo.');if(!payload.service_type)throw Error('Indica il tipo di servizio.');const dog=state.dogs.find(d=>d.id===payload.dog_id);if(!dog||dog.customer_id!==payload.customer_id)throw Error('L’animale selezionato non appartiene al cliente.');if(payload.employee_compensation>payload.customer_amount)throw Error('Il compenso dipendente non può superare l’importo cliente.');if(payload.payment_method!=='Altro')payload.payment_method_other=null;
...
window.archiveEntity=async(table,id,label)=>{
  if(!isAdmin())return;
  if(!confirm(`Eliminare ${label}? Verrà spostato nel Cestino e potrà essere ripristinato.`))return;
  try{
    await rpc('archive_entity',{p_table:table,p_id:id});
    await loadAll();
    const active=document.querySelector('#nav button.active')?.dataset.screen||'dashboard';
    render(active);
    toast('Elemento spostato nel Cestino');
  }catch(e){toast(e?.message||'Spostamento nel Cestino non riuscito')}
};
async function loadTrash(){state.trash=await rpc('list_trash')||[]}
async function renderTrash(){
  if(!isAdmin()){show('dashboard');return}
  await loadTrash();
  $('#trashList').innerHTML=state.trash.map(x=>`<details class="record-accordion trash-accordion"><summary><div class="record-summary-main"><span class="record-title">${esc(x.label)}</span><small>${esc(x.type_label)} · ${new Date(x.deleted_at).toLocaleString('it-IT')}</small></div><span class="accordion-chevron" aria-hidden="true"></span></summary><article class="record-expanded">${x.storage_path?`<p class="muted">Il PDF resta conservato finché non scegli “Elimina definitivamente”.</p>`:''}<div class="card-actions"><button class="primary" onclick="restoreEntity('${x.table_name}','${x.id}')">Ripristina</button>${isOwner()?`<button class="danger" onclick="purgeEntity('${x.table_name}','${x.id}','${esc(x.label)}')">Elimina definitivamente</button>`:''}</div></article></details>`).join('')||'<div class="card">Il Cestino è vuoto.</div>'
}
window.restoreEntity=async(table,id)=>{try{await rpc('restore_entity',{p_table:table,p_id:id});await loadAll();await renderTrash();toast('Elemento ripristinato')}catch(e){toast(e?.message||'Ripristino non riuscito')}};
async function deleteTrashStorageObjects(item){
  const related=(state.trash||[]).filter(x=>{
    if(item.table_name==='customers')return x.customer_id===item.id;
    if(item.table_name==='dogs')return x.dog_id===item.id;
    if(item.table_name==='dogsitter_services')return x.service_id===item.id;
    if(item.table_name==='dogsitter_quotes')return x.quote_id===item.id;
    return x.id===item.id&&x.table_name===item.table_name;
  });
  const paths=[...new Set(related.map(x=>x.storage_path).filter(Boolean))];
  for(const path of paths){
    const response=await fetch(`${C.SUPABASE_URL}/storage/v1/object/service-documents/${path}`,{method:'DELETE',headers:{apikey:C.SUPABASE_ANON_KEY,Authorization:`Bearer ${state.session.access_token}`}});
    if(!response.ok&&response.status!==404)throw Error(`Impossibile eliminare il file ${path} dallo Storage.`);
  }
}
window.purgeEntity=async(table,id,label)=>{
  if(!isOwner())return toast('Solo il Datore di lavoro può eliminare definitivamente dal Cestino.');
  const confirmation=prompt(`ELIMINAZIONE DEFINITIVA\n\n${label}\n\nQuesta operazione non può essere annullata.\n\nScrivi ELIMINA per confermare:`,'');
  if(confirmation!=='ELIMINA')return toast('Eliminazione annullata');
  const item=(state.trash||[]).find(x=>x.table_name===table&&x.id===id);
  try{
    if(table==='profiles'){
      const result=await invokeHyperHandler({action:'delete_user',user_id:id});
      await loadAll();await renderTrash();return toast(result.message||'Account eliminato definitivamente');
    }
    if(item)await deleteTrashStorageObjects(item);
    await rpc('purge_entity',{p_table:table,p_id:id});
    await loadAll();await renderTrash();toast('Elemento eliminato definitivamente');
  }catch(e){toast(e?.message||'Eliminazione definitiva non riuscita')}
};
...
window.viewService=id=>{
  ...
  const total=totalVisits(s),unit=Number(s.employee_unit_compensation||0)||Number(s.employee_compensation||0)/Math.max(1,total);
  const periods=normalizePeriods(s);
  const periodRows=periods.map((p,i)=>[
    dogDetailRow(`Periodo ${i+1} · Data inizio`,dateIT(p.start_date)),
    dogDetailRow(`Periodo ${i+1} · Data fine`,dateIT(p.end_date||p.start_date)),
    dogDetailRow(`Periodo ${i+1} · Uscite giornaliere`,p.daily_visits),
    dogDetailRow(`Periodo ${i+1} · Fascia oraria 1`,p.time_slot_1),
    dogDetailRow(`Periodo ${i+1} · Fascia oraria 2`,p.time_slot_2),
    dogDetailRow(`Periodo ${i+1} · Fascia oraria 3`,p.time_slot_3),
    dogDetailRow(`Periodo ${i+1} · Fascia oraria 4`,p.time_slot_4),
    dogDetailRow(`Periodo ${i+1} · Uscite totali`,periodVisits(p))
  ].join('')).join('');
  const rows=[
    dogDetailSection('Assegnazione',[
      dogDetailRow('Cliente',cname(s.customer_id)),
      dogDetailRow('Animale',dname(s.dog_id)),
      ...(isAdmin()?[dogDetailRow('Dipendente',pname(s.employee_id))]:[]),
      dogDetailRow('Tipo servizio',s.service_type),
      dogDetailRow('Stato servizio',statusLabels[s.status]||s.status),
      dogDetailRow('Stato cliente',s.client_status)
    ]),
    dogDetailSection('Periodi e orari',[periodRows||dogDetailRow('Periodi','Non indicati')]),
    dogDetailSection('Organizzazione',[
      dogDetailRow('Frequenza',s.frequency),
      dogDetailRow('Durata prevista',`${Number(s.planned_duration_minutes||0)} minuti`),
      dogDetailRow('Uscite totali',total),
      dogDetailRow('Chiavi affidate',s.keys_status),
      dogDetailRow('Consegna chiavi',s.keys_mode),
      dogDetailRow('Aggiornamenti al proprietario',s.customer_updates),
      dogDetailRow('Contatto veterinario in emergenza',s.auth_vet),
      dogDetailRow('Trasporto in clinica veterinaria',s.auth_transport),
      dogDetailRow('Note operative',s.operational_notes),
      dogDetailRow('Rapporto',s.report_text),
      dogDetailRow('Anomalie / note',s.incident_notes)
    ]),
    ...(isAdmin()?[dogDetailSection('Economia cliente',[
      dogDetailRow('Tariffa cliente per uscita',money(s.unit_rate||0)),
      dogDetailRow('Sconto',`${Number(s.discount_rate||0)}%`),
      dogDetailRow('Importo cliente',money(s.customer_amount||0)),
      dogDetailRow('Modalità di pagamento',s.payment_method_other||s.payment_method),
      dogDetailRow('Stato pagamento cliente',s.customer_payment_status==='incassato'?'Incassato':'Da incassare'),
      dogDetailRow('Acconto ricevuto',Number(s.deposit_amount||0)>0?money(s.deposit_amount):'Nessun acconto'),
      dogDetailRow('Data acconto',s.deposit_received_at?dateIT(s.deposit_received_at):null),
      dogDetailRow('Modalità acconto',s.deposit_payment_method),
      dogDetailRow('Riferimento / CRO',s.deposit_reference),
      dogDetailRow('Residuo cliente',money(s.balance_due||0))
    ]),dogDetailSection('Compenso dipendente',[
      dogDetailRow('Compenso per uscita',money(unit)),
      dogDetailRow('Compenso totale',money(s.employee_compensation||0)),
      dogDetailRow('Margine attività',money(Number(s.customer_amount||0)-Number(s.employee_compensation||0)))
    ])]:[dogDetailSection('Compenso',[
      dogDetailRow('Compenso per uscita',money(unit)),
      dogDetailRow('Compenso totale scheda',money(s.employee_compensation||0))
    ])])
  ].join('');
  modal('Dettaglio servizio',`<div class="entity-details">${rows}</div>`,async()=>{});
...
function documentTrashTable(d){
  if(d?.source_kind==='receipt')return 'dogsitter_receipts';
  if(d?.source_kind==='quote')return 'dogsitter_quote_document_versions';
  return 'dogsitter_document_versions';
}
window.deleteDocumentPermanently=async id=>{
  if(!isAdmin())return toast('Funzione riservata a Titolare/Vice.');
  const d=(state.documents||[]).find(x=>x.id===id);
  if(!d)return toast('Documento non trovato.');
  return archiveEntity(documentTrashTable(d),d.id,`${documentTypeLabel(d)} ${d.file_name||''}`.trim());
};

window.archiveDocument=async id=>{const d=state.documents.find(x=>x.id===id);if(d?.source_kind==='receipt')return toast('Le ricevute non usano l’archiviazione storico da questa schermata.');if(!confirm('Archiviare questo documento nello storico? Non verrà eliminato e non finirà nel Cestino.'))return;try{await rpc(d?.source_kind==='quote'?'archive_quote_document_version':'archive_document_version',{p_document_id:id});await loadAll();renderDocs();toast('Documento archiviato nello storico')}catch(e){toast(e.message)}};

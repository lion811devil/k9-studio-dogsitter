...
function reviewedPdfData(service,kind,draft={}){const base=servicePdfData(service);return {...base,...draft,service_id:service.id,document_type:kind,periods:normalizePeriods(service),period_runs:(state.periodRuns||[]).filter(r=>r.service_id===service.id)}}

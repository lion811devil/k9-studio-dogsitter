...
function servicePeriodWorkflowBlock(s){
  ...
  return `<div class="period-workflow"><strong>Gestione periodi</strong>${periods.map((p,i)=>{const r=runs.find(x=>Number(x.period_index)===i)||{status:'programmato',start_date:p.start_date,end_date:p.end_date};const today=localDate(),expired=r.status==='programmato'&&String(p.end_date||'')<today,canStart=r.status==='programmato'&&!expired&&!['chiuso','annullato'].includes(s.status),canEnd=r.status==='in_corso';return `<div class="period-workflow-row"><div><b>Periodo ${i+1}</b><span>${dateIT(p.start_date)} – ${dateIT(p.end_date)} · ${periodRunLabel(r.status)}</span>${r.started_at?`<small>Iniziato ${new Date(r.started_at).toLocaleString('it-IT')}</small>`:''}${r.ended_at?`<small>Terminato ${new Date(r.ended_at).toLocaleString('it-IT')}</small>`:''}</div><div>${expired?`<span class="muted">Periodo concluso · non avviato</span>`:''}${canStart?`<button type="button" class="primary" onclick="startServicePeriod('${s.id}',${i})">Inizia periodo</button>`:''}${canEnd?`<button type="button" class="primary" onclick="endServicePeriod('${s.id}',${i})">Termina periodo</button>`:''}</div></div>`}).join('')}</div>`
}
...
function renderPayments(){
  ...
  const toPay=maturedRows.filter(s=>s.employee_payment_status==='da_liquidare').reduce((a,s)=>a+Number(s.employee_compensation||0),0);
  const paidEmployees=maturedRows.filter(s=>s.employee_payment_status==='liquidato').reduce((a,s)=>a+Number(s.employee_compensation||0),0);
  ...
  $('#paymentList').innerHTML=rows.map(r=>{
    ...
    return `<details class="record-accordion payment-accordion"><summary><div class="record-summary-main"><span class="record-title">${esc(cname(r.customer_id))}</span><small>${esc(dname(r.dog_id))} · ${title}</small></div><strong class="record-amount">${money(r.total)}</strong><span class="accordion-chevron" aria-hidden="true"></span></summary><article class="record-expanded"><div class="entity-details">
      ...
      ${s&&matured(s)&&s.employee_payment_status==='da_liquidare'?`<button type="button" onclick="setEmployeePaid('${s.id}')">Segna dipendente liquidato</button>`:''}
    </div></article></details>`;

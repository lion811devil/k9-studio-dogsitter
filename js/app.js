...
const state={session:null,profile:null,customers:[],dogs:[],profiles:[],services:[],quotes:[],documents:[],communications:[],notifications:[],trash:[],settings:{...DEFAULT_SETTINGS}};
...
function unreadCommunicationCount(serviceId=null){return state.communications.filter(m=>(!serviceId||m.service_id===serviceId)&&m.status==='sent'&&!m.read_at&&m.author_id!==state.profile?.id).length}
...
async function loadAll(){if(isAdmin()){[state.customers,state.dogs,state.profiles,state.services,state.quotes,state.documents,state.communications,state.notifications]=await Promise.all([select('customers','select=*&deleted_at=is.null&order=last_name.asc,first_name.asc'),select('dogs','select=*&deleted_at=is.null&order=name.asc'),select('profiles','select=*&order=full_name.asc'),select('dogsitter_services','select=*&deleted_at=is.null&order=service_date.desc,service_time.asc'),loadQuotes(),loadDocuments(),select('service_communications','select=*&order=created_at.asc'),select('app_notifications','select=*&order=created_at.desc')])}else{const d=await rpc('employee_workspace');state.customers=d.customers||[];state.dogs=d.dogs||[];state.services=d.services||[];state.quotes=[];state.documents=[];state.profiles=[];state.communications=await select('service_communications','select=*&order=created_at.asc');state.notifications=await select('app_notifications','select=*&order=created_at.desc')}}
...
function attachmentSettingsBlock(){const rows=pdfAttachments().map(a=>`<div class="pdf-attachment-row"><div><strong>${esc(a.name)}</strong><small>${a.enabled===false?'Disattivato':'Allegato ai preventivi'}</small></div><div class="card-actions"><button type="button" onclick="togglePdfAttachment('${a.id}')">${a.enabled===false?'Attiva':'Disattiva'}</button><button type="button" class="danger" onclick="removePdfAttachment('${a.id}')">Rimuovi</button></div></div>`).join('')||'<p class="muted">Nessun PDF automatico caricato.</p>';return `<section class="settings-block"><h3>Allegati automatici ai preventivi</h3><p class="muted">Carica regolamento, privacy o condizioni. I PDF attivi saranno uniti in coda al preventivo generato.</p><label>Carica PDF<input id="pdfAttachmentFile" type="file" accept="application/pdf"></label><label class="checkbox-line"><input type="checkbox" name="attach_pdfs_to_quotes" ${state.settings.attach_pdfs_to_quotes!==false?'checked':''}> Allega automaticamente i PDF attivi ai preventivi</label><div class="pdf-attachment-list">${rows}</div></section>`}
async function saveAttachmentList(list){await request('/rest/v1/app_settings?id=eq.1',{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({pdf_attachments:list})});state.settings.pdf_attachments=list}
window.uploadPdfAttachment=async e=>{const file=e.target.files?.[0];if(!file)return;if(file.type!=='application/pdf')return toast('Seleziona un file PDF.');if(file.size>10*1024*1024)return toast('Il PDF supera 10 MB.');try{const id=crypto.randomUUID(),safe=file.name.replace(/[^a-zA-Z0-9._-]/g,'_'),path=`settings/${id}-${safe}`;const r=await fetch(`${C.SUPABASE_URL}/storage/v1/object/pdf-attachments/${path}`,{method:'POST',headers:{apikey:C.SUPABASE_ANON_KEY,Authorization:`Bearer ${state.session.access_token}`,'Content-Type':'application/pdf','x-upsert':'false'},body:file});const t=await r.text();if(!r.ok)throw Error(t||'Caricamento non riuscito');const list=[...pdfAttachments(),{id,name:file.name,path,enabled:true,created_at:new Date().toISOString()}];await saveAttachmentList(list);renderSettings();toast('PDF allegato salvato')}catch(err){toast(err.message)}};
window.togglePdfAttachment=async id=>{const list=pdfAttachments().map(a=>a.id===id?{...a,enabled:a.enabled===false}:a);await saveAttachmentList(list);renderSettings()};
window.removePdfAttachment=async id=>{const a=pdfAttachments().find(x=>x.id===id);if(!a||!confirm(`Rimuovere ${a.name}?`))return;try{await fetch(`${C.SUPABASE_URL}/storage/v1/object/pdf-attachments/${a.path}`,{method:'DELETE',headers:{apikey:C.SUPABASE_ANON_KEY,Authorization:`Bearer ${state.session.access_token}`}});await saveAttachmentList(pdfAttachments().filter(x=>x.id!==id));renderSettings();toast('Allegato rimosso')}catch(e){toast(e.message)}};

function renderSettings(){if(!isAdmin()){show('dashboard');return}const s=state.settings;$('#settingsPanel').innerHTML=`<form id="identityForm"><section class="settings-block"><h3>Logo</h3><div class="logo-preview"><img id="settingsLogoPreview" src="${esc(identityLogo())}" alt="Logo attivo"><div><label>Carica nuovo logo<input id="identityLogoFile" type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml"></label><p class="muted">Il logo attuale resta quello predefinito finché non ne carichi uno nuovo.</p><button type="button" id="restoreDefaultLogo">Ripristina logo predefinito</button></div></div><div class="settings-grid">${settingField('logo_size','Dimensione logo (28-160 px)','number')}<label>Posizione logo<select name="logo_position"><option value="sinistra" ${s.logo_position==='sinistra'?'selected':''}>Sinistra</option><option value="centro" ${s.logo_position==='centro'?'selected':''}>Centro</option><option value="destra" ${s.logo_position==='destra'?'selected':''}>Destra</option></select></label></div></section><section class="settings-block"><h3>Intestazione</h3><div class="settings-grid">${settingField('organization_name','Nome attività')}${settingField('app_name','Nome applicazione')}${settingField('subtitle','Sottotitolo', 'text', true)}${settingArea('description','Descrizione')}${settingField('address','Indirizzo')}${settingField('postal_code','CAP')}${settingField('city','Comune')}${settingField('province','Provincia')}${settingField('country','Nazione')}${settingField('phone','Telefono')}${settingField('mobile','Cellulare')}${settingField('email','Email','email')}${settingField('website','Sito web')}${settingField('vat_number','Partita IVA')}${settingField('fiscal_code','Codice fiscale')}${settingField('iban','IBAN', 'text', true)}${settingArea('social_text','Social e altri riferimenti')}</div></section><section class="settings-block"><h3>Grafica</h3><div class="settings-grid">${settingField('primary_color','Colore principale','color')}${settingField('secondary_color','Colore secondario','color')}${settingField('header_color','Colore intestazione','color')}${settingField('card_color','Colore schede','color')}${settingField('button_color','Colore pulsanti','color')}</div></section><section class="settings-block"><h3>PDF e piè di pagina</h3><div class="settings-grid">${settingArea('footer_text','Testo piè di pagina')}${settingArea('legal_text','Note legali / privacy')}${checkField('show_logo_pdf','Mostra logo nel PDF')}${checkField('show_header_pdf','Mostra intestazione nel PDF')}${checkField('show_footer_pdf','Mostra piè di pagina nel PDF')}${checkField('show_fiscal_data_pdf','Mostra dati fiscali nel PDF')}${checkField('show_qr_pdf','Mostra QR identificativo nel PDF')}${checkField('show_photos_pdf','Mostra fotografie nel PDF')}${checkField('show_signatures_pdf','Mostra firme nel PDF')}</div></section>${attachmentSettingsBlock()}<div class="settings-actions"><button class="primary" type="submit">Salva impostazioni</button><button type="button" id="resetIdentity">Ripristina tutti i valori iniziali</button></div></form>`;$('#identityForm').onsubmit=saveSettings;$('#identityLogoFile').onchange=previewAndUploadLogo;$('#pdfAttachmentFile').onchange=uploadPdfAttachment;$('#restoreDefaultLogo').onclick=()=>setLogoUrl('');$('#resetIdentity').onclick=resetSettings}
async function saveSettings(e){e.preventDefault();const f=new FormData(e.currentTarget),data={};for(const [k,v] of f.entries())data[k]=v;['show_logo_pdf','show_header_pdf','show_footer_pdf','show_fiscal_data_pdf','show_qr_pdf','show_photos_pdf','show_signatures_pdf','attach_pdfs_to_quotes'].forEach(k=>data[k]=f.has(k));data.logo_size=Math.max(28,Math.min(160,Number(data.logo_size||48)));await request('/rest/v1/app_settings?id=eq.1',{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(data)});state.settings={...state.settings,...data};applyIdentity();toast('Impostazioni salvate')}
...
function communicationStatusLabel(m){return m.status==='draft'?'Bozza':m.read_at?'Letta':'Inviata'}
...
window.openQuote=(id=null,customerId='',dogId='',sourceServiceId='')=>{
  ...
  modal(id?'Modifica preventivo':'Nuovo preventivo',`<div class="form-grid quote-form-grid">
  ...
  </div><section class="periods-section"><div class="section-head quote-section-head"><div><h3>Periodi e date</h3><p id="quotePeriodsSummary" class="muted"></p></div><button type="button" onclick="addPeriodRow('quote')">Aggiungi periodo</button></div><div id="quotePeriods">${quotePeriods.map((p,i)=>periodRow('quote',p,i)).join('')}</div></section><section class="quote-items-section"><div class="section-head quote-section-head"><div><h3>Servizio e tariffa</h3><p class="muted">Il numero totale delle uscite è calcolato automaticamente dai periodi.</p></div></div><div class="quote-simple-service"><label>Servizio<select id="quoteServicePreset" required>${quoteServicePresets.map(v=>`<option value="${esc(v)}" ${v===quoteService||quoteServiceCustom&&v==='Servizio personalizzato'?'selected':''}>${esc(v)}</option>`).join('')}</select></label><label id="quoteCustomServiceLabel" class="${quoteServiceCustom?'':'hidden'}">Descrizione personalizzata<input id="quoteCustomService" value="${esc(quoteServiceCustom&&quoteService!=='Servizio personalizzato'?quoteService:'')}" maxlength="120" placeholder="Descrivi il servizio"></label><label>Prezzo cliente per uscita (€)<input id="quoteUnitPrice" inputmode="decimal" type="number" min="0" step="0.01" value="${quoteUnitPrice}" required></label></div><div id="quotePeriodBreakdown" class="quote-period-breakdown hidden"></div><div class="quote-calculation"><div><span>Subtotale</span><strong id="quoteSubtotalPreview">0,00 €</strong></div><div><span>Sconto</span><strong id="quoteDiscountPreview">0,00 €</strong></div></div><div class="quote-total"><span>Totale preventivo</span><strong id="quoteTotalPreview">${money(q.total_amount||0)}</strong></div></section>`,async formData=>{
    ...
    if(unitPrice<0)throw Error('Il prezzo per uscita non può essere negativo.');
  ...
  const toggleQuoteService=()=>{const custom=$('#quoteServicePreset')?.value==='Servizio personalizzato';$('#quoteCustomServiceLabel')?.classList.toggle('hidden',!custom);if($('#quoteCustomService'))$('#quoteCustomService').required=custom};$('#quoteServicePreset').onchange=toggleQuoteService;toggleQuoteService();

...
const state={session:null,profile:null,customers:[],dogs:[],profiles:[],services:[],quotes:[],documents:[],communications:[],trash:[],settings:{...DEFAULT_SETTINGS}};
...
window.addPeriodRow=(kind,period={})=>{const host=document.getElementById(kind==='quote'?'quotePeriods':'servicePeriods');if(!host)return;host.insertAdjacentHTML('beforeend',periodRow(kind,period,host.querySelectorAll('.period-row').length));bindPeriodRows(kind);kind==='quote'?recalculateQuoteTotal():recalculateServiceTotals()};
window.removePeriodRow=(kind,button)=>{const host=document.getElementById(kind==='quote'?'quotePeriods':'servicePeriods');if(!host)return;if(host.querySelectorAll('.period-row').length<=1)return toast('Deve rimanere almeno un periodo.');button.closest('.period-row')?.remove();[...host.querySelectorAll('.period-row')].forEach((r,i)=>{r.dataset.periodIndex=i;const t=r.querySelector('.period-row-head strong');if(t)t.textContent=`Periodo ${i+1}`});kind==='quote'?recalculateQuoteTotal():recalculateServiceTotals()};
...
function buildNav(){const a=[['dashboard','Dashboard'],['customers','Clienti'],['dogs','Cani'],['employees','Utenti'],['services','Servizi'],['quotes','Preventivi'],['payments','Economia'],['documents','Documenti'],['audit','Log'],['trash','Cestino'],['profile','Pass'],['settings','Impostazioni']];const e=[['dashboard','Oggi'],['customers','Clienti assegnati'],['dogs','Cani assegnati'],['services','I miei servizi'],['compensation','Compensi'],['profile','Il mio pass']];const unread=unreadCommunicationCount();$('#nav').innerHTML=(isAdmin()?a:e).map(([id,l])=>`<button data-screen="${id}">${l}${id==='services'&&unread?`<span class="nav-notification">${unread}</span>`:''}</button>`).join('')}
...
async function loadAll(){if(isAdmin()){[state.customers,state.dogs,state.profiles,state.services,state.quotes,state.documents,state.communications]=await Promise.all([select('customers','select=*&deleted_at=is.null&order=last_name.asc,first_name.asc'),select('dogs','select=*&deleted_at=is.null&order=name.asc'),select('profiles','select=*&order=full_name.asc'),select('dogsitter_services','select=*&deleted_at=is.null&order=service_date.desc,service_time.asc'),loadQuotes(),loadDocuments(),select('service_communications','select=*&order=created_at.asc')])}else{const d=await rpc('employee_workspace');state.customers=d.customers||[];state.dogs=d.dogs||[];state.services=d.services||[];state.quotes=[];state.documents=[];state.profiles=[];state.communications=await select('service_communications','select=*&order=created_at.asc')}}
...
window.recalculateQuoteTotal=()=>{let visits=1;let days=1;try{const totals=periodsTotals(readPeriodRows('quote'));visits=Math.max(1,totals.visits);days=Math.max(1,totals.days)}catch{}const quantity=$('#quoteCalculatedVisits');if(quantity)quantity.value=visits;const unitPrice=Number($('#quoteUnitPrice')?.value||0);const subtotal=visits*unitPrice;const rate=Number($('#quoteDiscount')?.value||0);const discount=subtotal*rate/100;const total=subtotal-discount;if($('#quoteSubtotalPreview'))$('#quoteSubtotalPreview').textContent=money(subtotal);if($('#quoteDiscountPreview'))$('#quoteDiscountPreview').textContent=money(discount);if($('#quoteTotalPreview'))$('#quoteTotalPreview').textContent=money(total);const summary=$('#quotePeriodsSummary');if(summary)summary.textContent=`${days} giorni complessivi · ${visits} uscite totali`;return total};

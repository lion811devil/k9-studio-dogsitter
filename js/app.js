...
function renderNotifications(){
  ...
  host.innerHTML=state.notifications.map(n=>`<details class="record-accordion notification-card ${n.read_at?'':'unread'}"><summary><div class="record-summary-main"><span class="record-title">${esc(n.title||'Nuova notifica')}</span><small>${new Date(n.created_at).toLocaleString('it-IT')}</small></div>${n.read_at?'':`<span class="service-message-badge">Nuova</span>`}<span class="accordion-chevron"></span></summary><article class="record-expanded"><p class="notification-message">${esc(friendlyNotificationText(n))}</p><div class="card-actions">${n.service_id?`<button class="primary" onclick="openNotification('${n.id}','${n.service_id}')">Apri servizio</button>`:!n.read_at?`<button class="primary" onclick="markNotificationRead('${n.id}')">Segna come letta</button>`:''}<button class="danger" onclick="deleteAppNotification('${n.id}')">Elimina</button></div></article></details>`).join('')||'<div class="card">Nessuna notifica.</div>'
}

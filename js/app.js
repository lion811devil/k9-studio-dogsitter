...
function compensationPeriodCard(x){
...
function renderComp(){
  ...
  const maturedRows=compensationPeriodRows(),allRows=compensationPeriodRows({includeUnmatured:true});
  const defs=[['Oggi',x=>x.date===today],['Settimana',x=>x.date>=ws&&x.date<=today],['Mese',x=>String(x.date||'').startsWith(month)]];
  $('#compCards').innerHTML=defs.map(([l,f])=>{const t=compensationTotals(maturedRows.filter(f));return `<div class="stat"><span>${l}</span><strong>${money(t.sum)}</strong><small>${t.visits} uscite maturate</small></div>`}).join('')+`<div class="stat"><span>Da liquidare</span><strong>${money(maturedRows.filter(x=>x.payment_status==='da_liquidare').reduce((a,x)=>a+x.amount,0))}</strong><small>Periodi verificati</small></div>`;
  $('#compList').innerHTML=allRows.map(compensationPeriodCard).join('')||'<div class="card empty-state"><strong>Nessun compenso maturato</strong><span>I compensi diventano maturati quando un periodo terminato viene verificato dal Titolare/Vice.</span></div>'
}
...
function renderPayments(){
  ...
  const employeePeriodSection=`<div class="card"><div class="section-title-row"><div><h3>Compensi per periodo</h3><p class="muted">Il compenso matura solo dopo la verifica del singolo periodo.</p></div></div>${periodCompRows.map(x=>`<div class="linked-document-row"><div><strong>${esc(pname(x.service.employee_id))} · Periodo ${Number(x.run.period_index)+1}</strong><small>${esc(dname(x.service.dog_id))} · ${dateIT(x.period.start_date)} – ${dateIT(x.period.end_date)} · ${x.visits} uscite × ${money(x.unit)}</small></div><div><strong>${money(x.amount)}</strong><small>${x.payment_status==='liquidato'?'Liquidato':'Da liquidare'}</small>${x.payment_status==='da_liquidare'?`<button type="button" onclick="setPeriodEmployeePaid('${x.run.id}')">Segna liquidato</button>`:''}</div></div>`).join('')||'<p class="muted">Nessun periodo verificato.</p>'}</div>`;
...
window.downloadServiceReceipt=async id=>{try{if(!isAdmin())throw Error('Funzione riservata al titolare o vice amministratore.');const existing=state.documents.find(d=>d.source_kind==='receipt'&&d.service_id===id&&d.status!=='archived');if(existing){await downloadStoredDocument(existing.id);return}const meta=await rpc('create_service_receipt',{p_service_id:id});const blob=await makeServiceReceipt(id,false,meta);await uploadReceiptDocument(meta.storage_path,blob);await rpc('finalize_service_receipt',{p_receipt_id:meta.id});await loadAll();renderServices();downloadBlob(blob,meta.file_name);toast(`Ricevuta ${meta.receipt_number} emessa e archiviata`)}catch(e){toast(e.message||'Emissione ricevuta non riuscita')}};

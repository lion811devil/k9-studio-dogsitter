...
function renderPayments(){
  ...
  $('#paymentList').innerHTML=rows.map(r=>{
    ...
    return `<details class="record-accordion payment-accordion"><summary><div class="record-summary-main"><span class="record-title">${esc(cname(r.customer_id))}</span><small>${esc(dname(r.dog_id))} · ${title}</small></div><strong class="record-amount">${money(r.total)}</strong><span class="accordion-chevron" aria-hidden="true"></span></summary><article class="record-expanded"><div class="entity-details">
      ...
      ${s&&s.customer_payment_status!=='incassato'?`<button type="button" onclick="setCustomerPaid('${s.id}')">Segna cliente incassato</button>`:''}
...
function receiptDataForService(id,testMode=false){
  let service=state.services.find(x=>x.id===id);if(!service)throw Error('Servizio non trovato');service=serviceFinancialData(service);
  if(!testMode&&service.status!=='chiuso')throw Error('La ricevuta è disponibile solo dopo la chiusura del servizio.');
  if(!testMode&&service.customer_payment_status!=='incassato')throw Error('Prima di emettere la ricevuta, registra il pagamento del cliente come incassato.');
  const customer=state.customers.find(x=>x.id===service.customer_id)||{};
  const receiptDate=localDate();
  return {...service,customer_name:cname(service.customer_id),dog_name:dname(service.dog_id),customer_email:customer.email||'',customer_address:[customer.address,customer.city].filter(Boolean).join(', '),periods:normalizePeriods(service),receipt_date:receiptDate,receipt_number:`R-${receiptDate.replaceAll('-','')}-${String(service.id).slice(0,6).toUpperCase()}`,amount_received:Number(service.customer_amount||0),payment_method:service.payment_method_other||service.payment_method||service.deposit_payment_method||'—'};
}

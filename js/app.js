...
function show(id){const current=$$('.screen:not(.hidden)')[0]?.id||'';if(id==='services'&&current!=='services')resetServiceFilters();$$('.screen').forEach(e=>e.classList.add('hidden'));$('#'+id).classList.remove('hidden');$$('#nav button').forEach(b=>b.classList.toggle('active',b.dataset.screen===id));render(id)}
...
function linkedDocuments({customerId=null,dogId=null,serviceId=null}={}){
  ...
  return (state.documents||[]).filter(d=>{
    ...
    if(serviceId){
      const direct=d.service_id===serviceId;
      const quoteDoc=d.source_kind==='quote'&&quote&&d.quote_id===quote.id;
...
function customerCrmSections(customer,dogs,services){
  ...
  const docCounts={customer:documents.filter(d=>(d.document_type||'customer')==='customer').length,employee:documents.filter(d=>d.document_type==='employee').length,quote:documents.filter(d=>d.document_type==='quote').length,receipt:documents.filter(d=>d.document_type==='receipt').length};
  return `<section class="customer-crm" aria-label="CRM cliente">
    ...
    <details class="crm-section"><summary><span>Documenti</span><strong>${documents.length}</strong></summary><div class="crm-section-body"><div class="crm-mini-stats"><span>Cliente <b>${docCounts.customer}</b></span><span>Interni <b>${docCounts.employee}</b></span><span>Preventivi <b>${docCounts.quote}</b></span><span>Ricevute <b>${docCounts.receipt}</b></span></div>${documentHtml}</div></details>
...
function serviceCard(s){
  ...
  else{const serviceDocs=linkedDocuments({serviceId:s.id});const internalReceipt=serviceDocs.find(d=>d.document_type==='employee'&&d.status!=='archived'&&d.is_active!==false),receipt=serviceDocs.find(d=>d.document_type==='receipt'&&d.status!=='archived');const closed=s.status==='chiuso';if(closed){actions=`<button class="primary-soft" onclick="viewService('${s.id}')">Apri</button>${internalReceipt?`<button onclick="openDocument('${internalReceipt.id}')">Apri ricevuta interna</button>`:`<button class="primary" onclick="openPdfReviewHub('${s.id}')">Prepara ricevuta interna</button>`}${receipt?`<button class="primary" onclick="openDocument('${receipt.id}')">Visualizza ricevuta</button><button onclick="downloadStoredDocument('${receipt.id}')">Scarica ricevuta</button><button onclick="shareStoredDocument('${receipt.id}')">Condividi ricevuta</button>`:`<button class="primary" onclick="downloadServiceReceipt('${s.id}')">Emetti ricevuta</button>`}`;}else{actions=`<button class="primary-soft" onclick="viewService('${s.id}')">Apri</button><button onclick="addAssignedServiceToCalendar('${s.id}')">Collega a calendario</button><button class="communication-button" onclick="openServiceCommunications('${s.id}')">Comunicazioni${unreadCommunicationCount(s.id)?` <span>${unreadCommunicationCount(s.id)}</span>`:''}</button><button onclick="openService('${s.id}')">Modifica</button>${s.status==='da_verificare'?`<button class="primary" onclick="openPdfReviewHub('${s.id}')">Chiudi e prepara documenti</button>`:''}`;}}
...
function documentTypeLabel(d){if((d.document_type||'customer')==='quote')return 'Preventivo';if(d.document_type==='receipt')return 'Ricevuta';return d.document_type==='employee'?'Ricevuta interna':'PDF cliente'}
...
function closedServiceArchiveCard(s){
  ...
  const customer=docs.find(d=>(d.document_type||'customer')==='customer');
  const employee=docs.find(d=>d.document_type==='employee'&&!d.internal_receipt);
  const internal=docs.find(d=>d.document_type==='internal'||(d.document_type==='employee'&&d.internal_receipt));
  const receipt=docs.find(d=>d.document_type==='receipt');
  const one=(label,d,kind)=>d
    ? `<div class="archive-doc-row"><b>${label}</b><button onclick="downloadStoredDocument('${d.id}')">Scarica</button></div>`
    : `<div class="archive-doc-row"><b>${label}</b><button onclick="downloadClosedServiceDocument('${s.id}','${kind}')">Prepara e scarica</button></div>`;
  return `<details class="record-accordion document-accordion"><summary><div class="record-summary-main"><span class="record-title">${esc(cname(s.customer_id))} · ${esc(dname(s.dog_id))}</span><small>${esc(s.service_type||'Servizio')} · ${esc(servicePeriod(s))}</small></div><span class="pill">Chiuso</span><span class="accordion-chevron"></span></summary><article class="record-expanded document-card"><div class="archive-documents">${one('Documento cliente',customer||receipt,'customer')}${one('Documento dipendente',employee,'employee')}${one('Documento interno',internal,'internal')}</div><div class="service-actions"><button class="primary" onclick="downloadAllClosedServiceDocuments('${s.id}')">Scarica tutti e 3</button></div></article></details>`;
}
window.downloadClosedServiceDocument=async(id,kind)=>{try{
  ...
  if(kind==='customer'){const existing=state.documents.find(d=>d.service_id===id&&d.document_type==='receipt'&&d.status!=='archived');if(existing)return downloadStoredDocument(existing.id);return downloadServiceReceipt(id)}
  const docKind='employee';
  const meta=await rpc('create_document_version',{p_service_id:id,p_document_type:docKind});
  const base=reviewedPdfData(s,'employee',{});
  const data={...base,title:kind==='internal'?'Documento interno del servizio':'Documento compenso dipendente',internal_receipt:kind==='internal',employee_only:kind==='employee'};
  const blob=await window.K9PdfEngine.createServicePdf(data,'employee',state.settings||DEFAULT_SETTINGS,identityLogo());
  await uploadDocument(meta.storage_path,blob);await rpc('finalize_document_version',{p_document_id:meta.id});downloadBlob(blob,meta.file_name);await loadAll();
}catch(e){toast(e.message||'Documento non disponibile')}}
window.downloadAllClosedServiceDocuments=async id=>{try{
  if(!isAdmin())throw Error('Funzione riservata a Titolare/Vice.');
  await downloadClosedServiceDocument(id,'customer');
  await downloadClosedServiceDocument(id,'employee');
  await downloadClosedServiceDocument(id,'internal');
  toast('Preparati i tre documenti del servizio');
...
function renderDocs(){
  ...
  host.innerHTML=`<div class="document-summary"><div class="stat"><span>Documenti</span><strong>${docs.length}</strong></div><div class="stat"><span>Attivi</span><strong>${active}</strong></div><div class="stat"><span>PDF cliente legacy</span><strong>${customers}</strong></div><div class="stat"><span>Ricevute interne</span><strong>${employees}</strong></div><div class="stat"><span>Preventivi</span><strong>${quotes}</strong></div><div class="stat"><span>Ricevute</span><strong>${receipts}</strong></div><div class="stat"><span>Inviati</span><strong>${sent}</strong></div></div><div class="document-toolbar"><input id="documentSearch" type="search" placeholder="Cerca cliente, animale, dipendente o file"><select id="documentTypeFilter"><option value="">Tutti i documenti</option><option value="customer">PDF cliente</option><option value="employee">PDF dipendente</option><option value="quote">Preventivi</option><option value="receipt">Ricevute</option></select><select id="documentStatusFilter"><option value="">Tutti gli stati</option><option value="generated">Generati</option><option value="inviato">Inviati</option><option value="archived">Archiviati</option></select></div><div id="documentCards" class="document-grid"></div>`;
  const draw=()=>{
...
window.openPdfReviewHub=id=>{const s=state.services.find(x=>x.id===id);if(!s)return toast('Servizio non trovato');const allReady=s.status==='chiuso'||servicePeriodsReadyForClosure(id);modal('Documenti del servizio',`<div class="review-hub"><p><b>${esc(cname(s.customer_id))} · ${esc(dname(s.dog_id))}</b></p><p class="muted">${esc(servicePeriod(s))}</p>${!allReady?`<div class="modal-inline-error"><b>Periodo da completare:</b><br>${serviceClosurePendingPeriods(id).map(x=>`P${x.index} · ${dateIT(x.start_date)} – ${dateIT(x.end_date)} · ${periodRunLabel(x.status)}`).join('<br>')}<br><small>Apri il servizio e completa/verifica il periodo indicato prima di generare la ricevuta interna.</small></div>`:''}<div class="review-choice-grid"><button type="button" class="primary-soft" onclick="openPdfReview('${id}','employee')">Prepara ricevuta interna</button>${isAdmin()?`<button type="button" class="primary" onclick="downloadServiceReceipt('${id}')">Emetti ricevuta</button>`:''}</div><p class="muted"><b>Prepara ricevuta interna:</b> genera una vera ricevuta in formato amministrativo, archiviata come documento interno e visibile solo a titolare e vice; se il servizio è terminato viene chiuso automaticamente. <b>Emetti ricevuta:</b> emette la ricevuta definitiva collegata al pagamento cliente.</p></div>`,async()=>{});$('#modalSave')?.classList.add('hidden')}
window.openPdfReview=(id,kind)=>{const s=state.services.find(x=>x.id===id);if(!s)return toast('Servizio non trovato');if(kind==='customer')return toast('Il PDF cliente non è più previsto. Usa “Emetti ricevuta” per la ricevuta definitiva.');const d=pdfDraftFor(id,'employee'),quote=linkedQuoteForService(s),total=Number(quote?.total_amount??s.customer_amount??0),deposit=Number(quote?.deposit_amount??s.deposit_amount??0),balance=Math.max(0,total-deposit);modal('Ricevuta interna',`<div class="pdf-review-form form-grid"><label class="wide">Titolo documento<input name="title" value="${esc(d.title||'Ricevuta interna del servizio')}"></label><label>Totale servizio<input name="total_amount" type="number" step="0.01" min="0" value="${Number(d.total_amount??total)}"></label><label>Acconto ricevuto<input name="deposit_amount" type="number" step="0.01" min="0" value="${Number(d.deposit_amount??deposit)}"></label><label>Saldo<input name="saldo_amount" type="number" step="0.01" min="0" value="${Number(d.saldo_amount??balance)}"></label><label>Modalità pagamento<input name="payment_method" value="${esc(d.payment_method||quote?.payment_method||s.payment_method||'')}"></label><label class="wide">Causale / descrizione<textarea name="summary" rows="4">${esc(d.summary||`Corrispettivo per ${s.service_type||'servizio'} - ${servicePeriod(s)}`)}</textarea></label><label class="wide">Note interne<textarea name="operational_notes" rows="4">${esc(d.operational_notes||s.operational_notes||'')}</textarea></label><div class="wide pdf-review-options"><label class="checkbox-line"><input name="show_header" type="checkbox" value="1" ${d.show_header!==false?'checked':''}> Mostra intestazione</label><label class="checkbox-line"><input name="show_footer" type="checkbox" value="1" ${d.show_footer!==false?'checked':''}> Mostra piè di pagina</label></div></div><div class="pdf-review-actions"><button type="button" onclick="previewReviewedPdf('${id}','employee')">Anteprima ricevuta</button><button type="button" class="primary" onclick="generateReviewedPdf('${id}','employee')">Genera e archivia ricevuta interna</button></div>`,async()=>{const raw=currentReviewFormData('employee');await savePdfDraft(id,'employee',raw);toast('Bozza ricevuta interna salvata')})}
function currentReviewFormData(kind){const form=$('#modalForm');if(!form)return {};const raw=Object.fromEntries(new FormData(form));delete raw.update_service;raw.show_header=!!form.querySelector('[name="show_header"]')?.checked;raw.show_footer=!!form.querySelector('[name="show_footer"]')?.checked;raw.show_signatures=!!form.querySelector('[name="show_signatures"]')?.checked;if(kind==='employee'){raw.total_amount=Number(raw.total_amount||0);raw.deposit_amount=Number(raw.deposit_amount||0);raw.saldo_amount=Number(raw.saldo_amount||0)}return raw}
window.previewReviewedPdf=async(id,kind)=>{try{if(kind==='customer')throw Error('Il PDF cliente non è più previsto.');const s=state.services.find(x=>x.id===id),draft={...pdfDraftFor(id,'employee'),...currentReviewFormData('employee')};const base=receiptDataForService(id,false,{receipt_number:`INT-${String(s.id).slice(0,8).toUpperCase()}`,receipt_date:localDate()});const data={...base,...reviewedPdfData(s,'employee',draft),...draft,internal_receipt:true,test_mode:false};const blob=await window.K9PdfEngine.createReceiptPdf(data,state.settings||DEFAULT_SETTINGS,identityLogo(),'assets/timbro-firma-ricevuta.jpg?v=12923');const url=URL.createObjectURL(blob);window.open(url,'_blank','noopener');setTimeout(()=>URL.revokeObjectURL(url),60000)}catch(e){toast(e.message||'Anteprima non disponibile')}}
window.generateReviewedPdf=async(id,kind)=>{try{if(kind==='customer')throw Error('Il PDF cliente non è più previsto.');const s=state.services.find(x=>x.id===id);if(s.status!=='chiuso'&&!servicePeriodsReadyForClosure(id)){const pending=serviceClosurePendingPeriods(id);throw Error(`Completa prima ${pending.map(x=>`P${x.index} (${dateIT(x.start_date)}–${dateIT(x.end_date)}: ${periodRunLabel(x.status)})`).join(', ')}.`)};const draft={...pdfDraftFor(id,'employee'),...currentReviewFormData('employee')};await savePdfDraft(id,'employee',draft);const meta=await rpc('create_document_version',{p_service_id:id,p_document_type:'employee'});const base=receiptDataForService(id,false,{receipt_number:`INT-${String(meta.id||id).slice(0,8).toUpperCase()}`,receipt_date:localDate()});const data={...base,...reviewedPdfData(s,'employee',draft),...draft,internal_receipt:true,test_mode:false};const blob=await window.K9PdfEngine.createReceiptPdf(data,state.settings||DEFAULT_SETTINGS,identityLogo(),'assets/timbro-firma-ricevuta.jpg?v=12923');await uploadDocument(meta.storage_path,blob);await rpc('finalize_document_version',{p_document_id:meta.id});await update('dogsitter_services',id,{status:'chiuso',completed_at:new Date().toISOString()});await loadAll();renderServices();$('#modal')?.close?.();toast('Ricevuta interna generata e archiviata; servizio chiuso automaticamente')}catch(e){toast(e.message||'Generazione ricevuta interna non riuscita')}}

async function createAndArchiveDocumentPair(id){const docs=await rpc('create_document_pair',{p_service_id:id});for(const d of docs)await generateArchivedDocument(d,{download:false});await update('dogsitter_services',id,{status:'chiuso',completed_at:new Date().toISOString()});return docs}
...
window.setCustomerPaid=async id=>{
  ...
  const total=Number(quote.total_amount??rawService.customer_amount??0),deposit=Number(quote.deposit_amount||0),saldo=Math.max(0,total-deposit);
  const currentMethod=rawService.payment_method_other||rawService.payment_method||quote.payment_terms||'Bonifico';
  modal('Registra saldo cliente',`<div class="data-form-stack"><div class="protected-note"><b>Catena documentale</b><br>Preventivo ${quote.progressive?String(quote.progressive).padStart(3,'0'):'—'} → Servizio → Saldo → Ricevuta</div><div class="form-grid"><label>Totale preventivo<input value="${money(total)}" readonly></label><label>Acconto già ricevuto<input value="${money(deposit)}" readonly></label><label>Saldo da registrare<input value="${money(saldo)}" readonly></label><label>Modalità saldo<select id="finalPaymentMethod" name="payment_method">${paymentPresets.map(v=>`<option value="${esc(v)}" ${v===currentMethod?'selected':''}>${esc(v)}</option>`).join('')}</select></label><label id="finalPaymentOtherWrap" class="wide ${currentMethod==='Altro'?'':'hidden'}">Altra modalità<input name="payment_method_other" value="${esc(rawService.payment_method_other||'')}"></label></div><p class="muted">Confermando, il pagamento viene aggiornato sia nel servizio sia nel preventivo collegato. La ricevuta userà gli stessi importi del preventivo.</p></div>`,async formData=>{
    ...
    await loadAll();renderPayments();toast('Saldo registrato. Preventivo, servizio e ricevuta sono allineati.');
  });

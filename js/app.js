...
// Release 12.8.2: protezione runtime contro componenti legacy di recupero titolare.
(function installOwnerRecoveryCleanup(){
  const cleanup = () => {
    const selectors = [
      '#ownerRecoveryButton','#emergencyOwnerRecovery','#ownerRecoveryForm',
      '[data-action="owner-recovery"]','[data-action="emergency-owner-recovery"]',
      '.emergency-recovery-button','.owner-recovery-button'
    ];
    ...
    document.querySelectorAll('button,a').forEach(node => {
      if ((node.textContent || '').trim().toLowerCase() === 'recupero titolare') node.remove();
    });
  };
...
if('serviceWorker' in navigator){addEventListener('load',async()=>{try{const regs=await navigator.serviceWorker.getRegistrations();await Promise.all(regs.map(r=>r.unregister()));if('caches' in window){const keys=await caches.keys();await Promise.all(keys.map(k=>caches.delete(k)));}}catch(err){console.warn('Bonifica PWA:',err);}});}

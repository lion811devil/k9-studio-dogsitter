...
window.previewPdfAttachment=async id=>{const a=pdfAttachments().find(x=>x.id===id);if(!a)return toast('Allegato non trovato');try{const blob=await fetchPdfAttachmentBlob(a.path),url=URL.createObjectURL(blob);modal(`Anteprima · ${a.name}`,`<div class="pdf-preview-frame"><iframe src="${url}" title="Anteprima ${esc(a.name)}"></iframe></div>`,async()=>{});const save=$('#modalSave');if(save)save.classList.add('hidden');const dialog=$('#modal'),cleanup=()=>{URL.revokeObjectURL(url);dialog.removeEventListener('close',cleanup)};dialog.addEventListener('close',cleanup)}catch(e){toast(e.message)}};

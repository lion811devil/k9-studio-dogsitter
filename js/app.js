...
window.verifyServicePeriod=async(serviceId,index)=>{if(!isAdmin())return toast('Operazione riservata al Titolare/Vice.');if(!confirm(`Verificare e chiudere il periodo ${Number(index)+1}?`))return;try{await rpc('verify_service_period',{p_service_id:serviceId,p_period_index:Number(index)});await loadAll();renderServices();toast('Periodo verificato e chiuso')}catch(e){toast(e.message||'Verifica periodo non riuscita')}};

...
function documentTypeLabel(d){if((d.document_type||'customer')==='quote')return 'Preventivo';if(d.document_type==='receipt')return 'Ricevuta';return d.document_type==='employee'?'PDF dipendente':'PDF cliente'}
...
window.generateReviewedPdf=async(id,kind)=>{try{const s=state.services.find(x=>x.id===id);if(!servicePeriodsReadyForClosure(id))throw Error('Termina tutti i periodi prima della generazione definitiva.');const draft={...pdfDraftFor(id,kind),...currentReviewFormData(kind)};await savePdfDraft(id,kind,draft);const meta=await rpc('create_document_version',{p_service_id:id,p_document_type:kind});const data=reviewedPdfData(s,kind,draft);const pdfSettings={...(state.settings||DEFAULT_SETTINGS),show_header_pdf:draft.show_header!==false,show_footer_pdf:draft.show_footer!==false,show_signatures_pdf:draft.show_signatures===true};const blob=await window.K9PdfEngine.createServicePdf({...data,...meta},kind,pdfSettings,identityLogo());await uploadDocument(meta.storage_path,blob);await rpc('finalize_document_version',{p_document_id:meta.id});if(kind==='employee')await update('dogsitter_services',id,{status:'chiuso',completed_at:new Date().toISOString()});else{await loadAll();const docs=linkedDocuments({serviceId:id}).filter(d=>d.status!=='archived'&&d.is_active!==false);if(docs.some(d=>d.document_type==='customer')&&docs.some(d=>d.document_type==='employee'))await update('dogsitter_services',id,{status:'chiuso',completed_at:new Date().toISOString()})}await loadAll();renderServices();$('#modal')?.classList.add('hidden');toast(kind==='customer'?'PDF Cliente generato e archiviato':'Ricevuta interna completa generata; servizio chiuso e archiviato automaticamente')}catch(e){toast(e.message||'Generazione non riuscita')}}

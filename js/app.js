...
window.downloadClosedServiceDocument=async(id,kind,forceNew=false)=>{try{
  ...
  if(existing)return downloadStoredDocument(existing.id);

  const meta=await rpc('create_document_version',{p_service_id:id,p_document_type:kind});
  const financial=serviceFinancialData(s);
  const titles={
    customer:'Documento cliente del servizio',
    employee:'Documento compenso dipendente',
    internal:'Documento amministrativo interno'
  };
  const data={
    ...reviewedPdfData(s,kind,{title:titles[kind]}),
    ...meta,
    customer_amount:Number(financial.customer_amount||0),
    employee_compensation:Number(financial.employee_compensation||0),
    employee_payment_status:financial.employee_payment_status||'da_liquidare',
    customer_payment_status:financial.customer_payment_status||'da_incassare',
    deposit_amount:Number(financial.deposit_amount||0),
    balance_due:Number(financial.balance_due||0),
    internal_margin:Number(financial.customer_amount||0)-Number(financial.employee_compensation||0)
  };
  ...
  downloadBlob(blob,meta.file_name);
  await loadAll();
}catch(e){toast(e.message||'Documento non disponibile')}}
window.downloadAllClosedServiceDocuments=async id=>{try{
  if(!isAdmin())throw Error('Funzione riservata a Titolare/Vice.');
  await downloadClosedServiceDocument(id,'customer',true);
  await new Promise(r=>setTimeout(r,250));
  await downloadClosedServiceDocument(id,'employee',true);
  await new Promise(r=>setTimeout(r,250));
  await downloadClosedServiceDocument(id,'internal',true);
  toast('Creati e scaricati tre documenti distinti');

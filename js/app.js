...
function attachmentSettingsBlock(){const rows=pdfAttachments().map(a=>`<div class="pdf-attachment-row"><div><strong>${esc(a.name)}</strong><small>${a.enabled===false?'Disattivato':'Allegato ai preventivi'}</small></div><div class="card-actions"><button type="button" onclick="togglePdfAttachment('${a.id}')">${a.enabled===false?'Attiva':'Disattiva'}</button><button type="button" class="danger" onclick="removePdfAttachment('${a.id}')">Rimuovi</button></div></div>`).join('')||'<p class="muted">Nessun PDF automatico caricato.</p>';return `<p class="muted">Carica regolamento, privacy o condizioni. I PDF attivi saranno uniti in coda al preventivo generato.</p><label>Carica PDF<input id="pdfAttachmentFile" type="file" accept="application/pdf"></label><label class="checkbox-line"><input type="checkbox" name="attach_pdfs_to_quotes" ${state.settings.attach_pdfs_to_quotes!==false?'checked':''}> Allega automaticamente i PDF attivi ai preventivi</label><div class="pdf-attachment-list">${rows}</div>`}
...
window.uploadPdfAttachment=async e=>{const file=e.target.files?.[0];if(!file)return;e.target.value='';if(file.type!=='application/pdf')return toast('Seleziona un file PDF valido.');if(file.size>10*1024*1024)return toast('Il PDF supera 10 MB.');let path='';try{const id=crypto.randomUUID(),safe=file.name.replace(/[^a-zA-Z0-9._-]/g,'_'),newPath=`settings/${id}-${safe}`;path=newPath;const r=await fetch(`${C.SUPABASE_URL}/storage/v1/object/pdf-attachments/${newPath}`,{method:'POST',headers:{apikey:C.SUPABASE_ANON_KEY,Authorization:`Bearer ${state.session.access_token}`,'Content-Type':'application/pdf','x-upsert':'false'},body:file});const t=await r.text();if(!r.ok)throw Error(t||'Caricamento non riuscito');const list=[...pdfAttachments(),{id,name:file.name,path:newPath,enabled:true,created_at:new Date().toISOString()}];try{await saveAttachmentList(list)}catch(err){await removeStorageObject('pdf-attachments',newPath).catch(()=>{});throw err}renderSettings();toast('PDF allegato salvato')}catch(err){toast(err.message||'Caricamento allegato non riuscito')}};

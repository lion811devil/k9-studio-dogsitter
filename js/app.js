...
function receiptDataForService(id,testMode=false){
  ...
  const quoteNumber=quoteProgressive?`${String(quoteProgressive).padStart(3,'0')}/${year}`:`${String(quote?.id||service.id).slice(0,8).toUpperCase()}/${year}`;
  const receiptNumber=quoteProgressive?`R-${String(quoteProgressive).padStart(3,'0')}/${year}`:`R-${String(quote?.id||service.id).slice(0,8).toUpperCase()}/${year}`;
  const total=Number(quote?.total_amount??service.customer_amount??0);
  const deposit=Number(quote?.deposit_amount??service.deposit_amount??0);
  const saldo=Math.max(0,total-deposit);
  const periods=quote?normalizePeriods(quote,'quote'):normalizePeriods(service);
  const paymentMethod=service.payment_method_other||service.payment_method||quote?.payment_terms||service.deposit_payment_method||quote?.deposit_payment_method||'—';
  return {...service,quote_id:quote?.id||service.quote_id||null,quote_number:quoteNumber,quote_date:quoteDate,customer_name:cname(service.customer_id),dog_name:dname(service.dog_id),customer_email:customer.email||'',customer_phone:customer.phone||'',customer_address:[customer.address,customer.postal_code,customer.city].filter(Boolean).join(', '),periods,receipt_date:receiptDate,receipt_number:receiptNumber,total_amount:total,deposit_amount:deposit,saldo_amount:saldo,amount_received:total,payment_method:paymentMethod,deposit_payment_method:quote?.deposit_payment_method||service.deposit_payment_method||null,deposit_received_at:quote?.deposit_received_at||service.deposit_received_at||null,test_mode:!!testMode};
}
async function makeServiceReceipt(id,testMode=false){
  if(!window.K9PdfEngine?.createReceiptPdf)throw Error('Motore ricevuta non disponibile. Aggiorna la pagina.');
  return window.K9PdfEngine.createReceiptPdf(receiptDataForService(id,testMode),state.settings||DEFAULT_SETTINGS,identityLogo(),'assets/timbro-firma-ricevuta.jpg?v=1296');
}
...
window.downloadServiceReceipt=async id=>{try{const data=receiptDataForService(id),blob=await makeServiceReceipt(id);downloadBlob(blob,`Ricevuta_${String(data.customer_name||'cliente').replace(/[^a-z0-9àèéìòù_-]+/gi,'_')}_${data.receipt_date}.pdf`);toast('Ricevuta cliente generata')}catch(e){toast(e.message||'Generazione ricevuta non riuscita')}};
window.shareServiceReceipt=async id=>{try{const data=receiptDataForService(id),blob=await makeServiceReceipt(id),name=`Ricevuta_${String(data.customer_name||'cliente').replace(/[^a-z0-9àèéìòù_-]+/gi,'_')}_${data.receipt_date}.pdf`,file=new File([blob],name,{type:'application/pdf'});if(navigator.canShare?.({files:[file]}))await navigator.share({title:'Ricevuta servizio',text:`Ricevuta del servizio per ${data.customer_name}.`,files:[file]});else{downloadBlob(blob,name);toast('Condivisione file non disponibile: ricevuta scaricata')}}catch(e){if(e.name!=='AbortError')toast(e.message||'Condivisione ricevuta non riuscita')}};

...
async function bootstrap(){const p=await select('profiles',`select=*&id=eq.${state.session.user.id}&limit=1`);state.profile=p[0];if(!state.profile||!state.profile.active)throw Error('Account non attivo.');await loadSettings();applyIdentity();$('#auth').classList.add('hidden');$('#app').classList.remove('hidden');$('#roleLabel').textContent=`${state.profile.full_name||state.profile.email} · ${roleLabels[state.profile.role]||state.profile.role}`;buildNav();try{await rpc('refresh_operational_reminders')}catch(e){console.warn('Promemoria operativi:',e.message)}await loadAll();buildNav();startNotificationPolling();show('dashboard')}
...
function customerCrmSections(customer,dogs,services){
  ...
  const totalServices=serviceRows.reduce((sum,s)=>sum+Number(s.customer_amount||0),0);
  const incassato=serviceRows.filter(s=>s.customer_payment_status==='incassato').reduce((sum,s)=>sum+Number(s.customer_amount||0),0);
  const daIncassare=serviceRows.filter(s=>s.customer_payment_status==='da_incassare').reduce((sum,s)=>sum+Number(s.customer_amount||0),0);
  const acconti=quotes.reduce((sum,q)=>sum+Number(q.deposit_amount||0),0);
  const dogRows=dogs.map(d=>`<div class="crm-row"><div><strong>${esc(d.name)}</strong><small>${esc(firstValue(d,'breed_detail','breed','breed_type')||'Razza non indicata')}</small></div><button type="button" onclick="openDog('${d.id}')">Apri cane</button></div>`).join('')||'<div class="empty-compact">Nessun cane associato.</div>';
  const serviceHtml=serviceRows.map(s=>`<div class="crm-row"><div><strong>${esc(dname(s.dog_id))} · ${esc(s.service_type||'Servizio')}</strong><small>${esc(servicePeriod(s))} · ${esc(customerCrmStatusLabel(s.status))}</small></div><div class="crm-row-actions"><button type="button" onclick="viewService('${s.id}')">Dettaglio</button><button type="button" onclick="openServiceCommunications('${s.id}')">Comunicazioni${unreadCommunicationCount(s.id)?` (${unreadCommunicationCount(s.id)})`:''}</button></div></div>`).join('')||'<div class="empty-compact">Nessun servizio collegato.</div>';
  ...
  const communicationHtml=communications.slice(0,12).map(m=>{const service=serviceRows.find(s=>s.id===m.service_id);return `<div class="crm-row"><div><strong>${esc(communicationLabel(m))}</strong><small>${esc(dname(service?.dog_id))} · ${communicationTime(m.created_at)} · ${esc(String(m.message||'').slice(0,90))}${String(m.message||'').length>90?'…':''}</small></div><button type="button" onclick="openServiceCommunications('${m.service_id}')">Apri</button></div>`}).join('')||'<div class="empty-compact">Nessuna comunicazione collegata.</div>';
  return `<section class="customer-crm" aria-label="CRM cliente">
    ...
    <details class="crm-section"><summary><span>Servizi</span><strong>${serviceRows.length}</strong></summary><div class="crm-section-body">${serviceHtml}</div></details>
    <details class="crm-section"><summary><span>Preventivi</span><strong>${quotes.length}</strong></summary><div class="crm-section-body">${quoteHtml}</div></details>
    <details class="crm-section"><summary><span>Documenti</span><strong>${documents.length}</strong></summary><div class="crm-section-body">${documentHtml}</div></details>
    <details class="crm-section"><summary><span>Comunicazioni</span><strong>${communications.length}</strong></summary><div class="crm-section-body">${communicationHtml}</div></details>
    <details class="crm-section"><summary><span>Situazione economica</span><strong>${money(daIncassare)}</strong></summary><div class="crm-section-body"><div class="crm-economy"><div><span>Servizi complessivi</span><strong>${money(totalServices)}</strong></div><div><span>Incassato</span><strong>${money(incassato)}</strong></div><div><span>Da incassare</span><strong>${money(daIncassare)}</strong></div><div><span>Acconti preventivi</span><strong>${money(acconti)}</strong></div></div></div></details>
  </section>`;
}
...
function auditSubject(a){const d=a.details?.new||a.details?.old||{};if(a.table_name==='customers')return [d.first_name,d.last_name].filter(Boolean).join(' ');if(a.table_name==='dogs')return d.name||'';if(a.table_name==='dogsitter_services')return d.service_type||'';if(a.table_name==='dogsitter_quotes')return d.customer_name||d.payment_status||'';if(a.table_name==='profiles')return d.full_name||d.email||'';return ''}function auditDetails(a){const d=a.details?.new||a.details?.old||{};const hidden=new Set(['id','created_by','updated_at','created_at','customer_id','employee_id','dog_id','deleted_by']);return Object.entries(d).filter(([k,v])=>!hidden.has(k)&&v!==null&&v!=='').slice(0,12).map(([k,v])=>`<div><b>${esc(k.replaceAll('_',' '))}:</b> ${esc(typeof v==='object'?JSON.stringify(v):v)}</div>`).join('')||'<div>Nessun dettaglio aggiuntivo.</div>'}async function renderAudit(){
  ...
  const tableNames={customers:'Cliente',dogs:'Cane',dogsitter_services:'Servizio',dogsitter_quotes:'Preventivo',profiles:'Utente'},actionNames={INSERT:'Creato',UPDATE:'Modificato',DELETE:'Eliminato',PAYMENT_SYNC:'Sincronizzato pagamento'};

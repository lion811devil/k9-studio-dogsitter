...
function parseAuthCallback(){
  ...
  if(type==='recovery'&&accessToken){
    return {type,tokenHash:'',session:{access_token:accessToken,refresh_token:refreshToken||'',expires_in:expiresIn,expires_at:Math.floor(Date.now()/1000)+expiresIn,token_type:hash.get('token_type')||'bearer'}};
  }
  return {type,tokenHash,session:null,error:hash.get('error_description')||query.get('error_description')||hash.get('error')||query.get('error')||''};
}
async function verifyRecoveryTokenHash(tokenHash){
  const r=await fetch(`${C.SUPABASE_URL}/auth/v1/verify`,{method:'POST',headers:{apikey:C.SUPABASE_ANON_KEY,'Content-Type':'application/json'},body:JSON.stringify({type:'recovery',token_hash:tokenHash})});
  const text=await r.text();let data={};try{data=text?JSON.parse(text):{}}catch{data={message:text}}
  if(!r.ok)throw Error(data?.msg||data?.message||data?.error_description||'Link di recupero non valido o scaduto.');
  if(!data?.access_token)throw Error('Supabase non ha restituito una sessione di recupero valida.');
  return data;
}
...
async function initializePasswordRecovery(){
  ...
  if(callback.type==='recovery'&&callback.tokenHash){
    try{const session=await verifyRecoveryTokenHash(callback.tokenHash);showPasswordRecovery(session);return true}
    catch(error){$('#authError').textContent=error?.message||'Link di recupero non valido o scaduto.';clearAuthCallbackUrl();return false}
  }
  return false;
}
...
function showOwnerRecovery(){
  ...
  $('#ownerRecoveryForm')?.classList.remove('hidden');
  $('#ownerRecoveryError').textContent='';
  const loginEmail=$('#loginEmail')?.value.trim();
  if(loginEmail&&!$('#ownerRecoveryEmail').value)$('#ownerRecoveryEmail').value=loginEmail;
  $('#ownerRecoveryEmail')?.focus();
}
function hideOwnerRecovery(){
  $('#ownerRecoveryForm')?.classList.add('hidden');
  $('#loginForm')?.classList.remove('hidden');
  $('#ownerRecoveryError').textContent='';
}
async function emergencyOwnerRecovery(email,recoveryCode,password){
  const r=await fetch(`${C.SUPABASE_URL}/functions/v1/hyper-handler`,{
    method:'POST',
    headers:{apikey:C.SUPABASE_ANON_KEY,Authorization:`Bearer ${C.SUPABASE_ANON_KEY}`,'Content-Type':'application/json','X-Request-Id':crypto.randomUUID?.()||String(Date.now())},
    body:JSON.stringify({action:'emergency_owner_recovery',email,recovery_code:recoveryCode,password})
  });
  const text=await r.text();let data={};try{data=text?JSON.parse(text):{}}catch{data={message:text}}
  if(!r.ok)throw Error(data?.message||`Recupero titolare non riuscito (${r.status}).`);
  localStorage.removeItem('k9_session');state.session=null;
  $('#ownerRecoveryForm')?.classList.add('hidden');$('#loginForm')?.classList.remove('hidden');
  $('#loginEmail').value=email;$('#loginPassword').value='';
  $('#ownerRecoveryCode').value='';$('#ownerRecoveryPassword').value='';$('#ownerRecoveryPasswordConfirm').value='';
  $('#authError').textContent='Accesso titolare ripristinato. Accedi con la nuova password e disattiva subito il recupero di emergenza in Supabase.';
  return data;
}
...
$('#emergencyOwnerRecovery').addEventListener('click',()=>{if(!configured())return $('#authError').textContent='Configura config.js.';showOwnerRecovery()});
$('#cancelOwnerRecovery').addEventListener('click',hideOwnerRecovery);
$('#ownerRecoveryForm').addEventListener('submit',async e=>{e.preventDefault();$('#ownerRecoveryError').textContent='';const submit=e.currentTarget.querySelector('button.primary');try{const email=$('#ownerRecoveryEmail').value.trim().toLowerCase(),code=$('#ownerRecoveryCode').value,password=$('#ownerRecoveryPassword').value,confirmPassword=$('#ownerRecoveryPasswordConfirm').value;if(!email)throw Error('Inserisci l’email del titolare.');if(code.length<24)throw Error('Il codice di recupero deve contenere almeno 24 caratteri.');if(password.length<12)throw Error('La nuova password deve contenere almeno 12 caratteri.');if(password!==confirmPassword)throw Error('Le due password non coincidono.');if(!confirm('Confermi il recupero di emergenza dell’account Titolare?'))return;submit.disabled=true;submit.textContent='Ripristino in corso…';await emergencyOwnerRecovery(email,code,password)}catch(err){$('#ownerRecoveryError').textContent=err.message}finally{submit.disabled=false;submit.textContent='Ripristina accesso titolare'}});

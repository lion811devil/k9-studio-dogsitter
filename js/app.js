...
const RECOVERY_REDIRECT_URL=()=>`${APP_BASE_URL()}?password-recovery=1`;
function parseAuthCallback(){
  ...
  const declaredType=hash.get('type')||query.get('type')||'';
  const type=declaredType||(recoveryRequested&&accessToken?'recovery':'');
  ...
  if(type==='recovery'&&accessToken){
    return {type,session:{access_token:accessToken,refresh_token:refreshToken,expires_in:expiresIn,expires_at:Math.floor(Date.now()/1000)+expiresIn,token_type:hash.get('token_type')||query.get('token_type')||'bearer'},error:''};
  }
  if(recoveryRequested&&!error&&!accessToken){
    return {type:'recovery',session:null,error:'Il collegamento di recupero non contiene una sessione valida oppure è scaduto. Richiedi una nuova email.'};
  }
  return {type,session:null,error};
}
function clearAuthCallbackUrl(){history.replaceState({},document.title,APP_BASE_URL())}

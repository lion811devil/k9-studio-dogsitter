...
function serviceCard(s){
  ...
  else{const serviceDocs=linkedDocuments({serviceId:s.id});const hasEmployee=serviceDocs.some(d=>d.document_type==='employee'&&d.status!=='archived'&&d.is_active!==false),receipt=serviceDocs.find(d=>d.document_type==='receipt'&&d.status!=='archived'),receiptEligible=s.status==='chiuso'&&s.customer_payment_status==='incassato'&&!!linkedQuoteForService(s);actions=`<button class="primary-soft" onclick="viewService('${s.id}')">Apri</button><button onclick="addAssignedServiceToCalendar('${s.id}')">Collega a calendario</button><button class="communication-button" onclick="openServiceCommunications('${s.id}')">Comunicazioni${unreadCommunicationCount(s.id)?` <span>${unreadCommunicationCount(s.id)}</span>`:''}</button><button onclick="openService('${s.id}')">Modifica</button><button onclick="duplicateService('${s.id}')">Duplica</button><button onclick="openQuote(null,'${s.customer_id}','${s.dog_id}','${s.id}')">Preventivo</button>${s.status==='da_verificare'?`<button class="primary" onclick="openPdfReviewHub('${s.id}')">Revisiona PDF e chiudi</button>`:''}${s.status==='chiuso'&&!hasEmployee?`<button class="primary" onclick="openPdfReviewHub('${s.id}')">Prepara ricevuta interna</button>`:''}${s.status==='chiuso'&&hasEmployee?`<button onclick="openPdfReview('${s.id}','employee')">Nuova versione ricevuta interna</button>`:''}${receipt?`<button class="primary" onclick="openDocument('${receipt.id}')">Visualizza ricevuta</button><button onclick="downloadStoredDocument('${receipt.id}')">Scarica ricevuta</button><button onclick="shareStoredDocument('${receipt.id}')">Condividi ricevuta</button>`:isAdmin()?`<button class="primary" onclick="downloadServiceReceipt('${s.id}')">Emetti ricevuta</button>`:''}<button class="danger" onclick="archiveEntity('dogsitter_services','${s.id}','servizio')">Elimina</button>`}
...
function servicePeriodsReadyForClosure(id){const runs=(state.periodRuns||[]).filter(r=>r.service_id===id);return runs.length>0&&runs.every(r=>['da_verificare','chiuso'].includes(r.status))}

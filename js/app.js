...
const auditFieldLabels={status:'Stato',periods:'Periodi',auth_vet:'Autorizzazione veterinaria',auth_transport:'Trasporto veterinario',end_date:'Data fine',start_date:'Data inizio',quote_id:'Preventivo collegato',frequency:'Frequenza',unit_rate:'Tariffa cliente',balance_due:'Residuo',keys_status:'Chiavi affidate',daily_visits:'Prestazioni giornaliere',service_date:'Data servizio',service_time:'Orario indicativo',deposit_amount:'Acconto',deposit_date:'Data acconto',deposit_method:'Modalità acconto',deposit_reference:'Riferimento acconto',customer_payment_status:'Stato pagamento',customer_amount:'Importo cliente',employee_compensation:'Compenso dipendente',service_type:'Tipo servizio',payment_method:'Metodo di pagamento'};
function auditValueLabel(key,value){
  ...
  if(['start_date','end_date','service_date','deposit_date'].includes(key))return dateIT(value);
  ...
  const labels={programmato:'Programmato',in_corso:'In corso',da_verificare:'Da verificare',chiuso:'Chiuso',annullato:'Annullato',da_incassare:'Da incassare',incassato:'Incassato'};
  return labels[value]||String(value).replaceAll('_',' ');
}
function auditDetails(a){
  const before=a.details?.old||{},after=a.details?.new||{};
  const hidden=new Set(['id','created_by','updated_at','created_at','customer_id','employee_id','dog_id','deleted_by','updated_by']);
  ...
  const rows=(a.action==='UPDATE'||a.action==='PAYMENT_SYNC'?changed:keys.filter(k=>after[k]!==undefined&&after[k]!==null&&after[k]!=='' )).slice(0,14);
  if(!rows.length)return '<div class="audit-empty">Nessun dettaglio aggiuntivo.</div>';
  return rows.map(k=>{const label=auditFieldLabels[k]||k.replaceAll('_',' ').replace(/^./,x=>x.toUpperCase());const oldVal=auditValueLabel(k,before[k]);const newVal=auditValueLabel(k,after[k]);if((a.action==='UPDATE'||a.action==='PAYMENT_SYNC')&&before[k]!==undefined)return `<div class="audit-change"><span>${esc(label)}</span><strong>${oldVal} <b aria-hidden="true">→</b> ${newVal}</strong></div>`;return `<div class="audit-change"><span>${esc(label)}</span><strong>${newVal}</strong></div>`}).join('');
}
async function renderAudit(){
  ...
  const tableNames={customers:'Cliente',dogs:'Cane',dogsitter_services:'Servizio',dogsitter_quotes:'Preventivo',profiles:'Utente'},actionNames={INSERT:'Creato',UPDATE:'Modificato',DELETE:'Archiviato',PAYMENT_SYNC:'Sincronizzato pagamento'};
  $('#auditList').innerHTML=rows.map(a=>{const who=state.profiles.find(p=>p.id===a.user_id)?.full_name||'Sistema';const obj=tableNames[a.table_name]||a.table_name||'Elemento';const subject=auditSubject(a);const title=`${actionNames[a.action]||a.action} ${obj.toLowerCase()}${subject?`: ${subject}`:''}`;return `<details class="record-accordion audit-accordion"><summary><div class="record-summary-main"><span class="record-title">${esc(title)}</span><small>${new Date(a.created_at).toLocaleString('it-IT')}</small></div><span class="pill">${esc(who)}</span><span class="accordion-chevron" aria-hidden="true"></span></summary><article class="record-expanded"><div class="audit-friendly">${auditDetails(a)}</div></article></details>`}).join('')||'<div class="card">Nessuna attività registrata.</div>'
}

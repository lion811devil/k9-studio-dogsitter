...
window.recalculateQuoteTotal=()=>{let visits=1;try{const periods=readPeriodRows('quote');visits=Math.max(1,periodsTotals(periods).visits)}catch{}const rows=$$('.quote-item-row');rows.forEach(row=>{const q=row.querySelector('[name="quote_item_quantity"]');if(q)q.value=visits});const subtotal=rows.reduce((sum,row)=>sum+visits*Number(row.querySelector('[name="quote_item_unit_price"]')?.value||0),0);const rate=Number($('#quoteDiscount')?.value||0);const discount=subtotal*rate/100;const total=subtotal-discount;if($('#quoteSubtotalPreview'))$('#quoteSubtotalPreview').textContent=money(subtotal);if($('#quoteDiscountPreview'))$('#quoteDiscountPreview').textContent=money(discount);if($('#quoteTotalPreview'))$('#quoteTotalPreview').textContent=money(total);const summary=$('#quotePeriodsSummary');if(summary){try{const t=periodsTotals(readPeriodRows('quote'));summary.textContent=`${t.days} giorni complessivi · ${t.visits} uscite totali`}catch{summary.textContent='Completa i periodi'}}return total};
window.openQuote=(id=null,customerId='',dogId='',sourceServiceId='')=>{
  ...
  const rows=q.quote_items?.length?q.quote_items:[{description:source?.service_type||'Dog Walking',quantity:quoteOutings,unit_price:Number(source?.unit_rate||source?.customer_amount||0)}];
  ...
  </div><section class="periods-section"><div class="section-head quote-section-head"><div><h3>Periodi e date</h3><p id="quotePeriodsSummary" class="muted"></p></div><button type="button" onclick="addPeriodRow('quote')">Aggiungi periodo</button></div><div id="quotePeriods">${quotePeriods.map((p,i)=>periodRow('quote',p,i)).join('')}</div></section><section class="quote-items-section"><div class="section-head quote-section-head"><h3>Voci del preventivo</h3><button type="button" onclick="addQuoteItem()">Aggiungi voce</button></div><div id="quoteItems">${rows.map(quoteItemRow).join('')}</div><div class="quote-calculation"><div><span>Subtotale</span><strong id="quoteSubtotalPreview">0,00 €</strong></div><div><span>Sconto</span><strong id="quoteDiscountPreview">0,00 €</strong></div></div><div class="quote-total"><span>Totale preventivo</span><strong id="quoteTotalPreview">${money(q.total_amount||0)}</strong></div></section>`,async formData=>{
    const periods=readPeriodRows('quote');const periodTotal=periodsTotals(periods);
    const itemRows=$$('.quote-item-row');
    const items=itemRows.map((row,index)=>{const preset=row.querySelector('[name="quote_item_service"]')?.value||'';const custom=String(row.querySelector('[name="quote_item_custom"]')?.value||'').trim();const description=preset==='Servizio personalizzato'?custom:preset;return {description,quantity:periodTotal.visits,unit_price:Number(row.querySelector('[name="quote_item_unit_price"]')?.value||0),position:index+1}});

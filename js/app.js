...
async function deleteStorageObjectIfPresent(d){
  if(!d?.storage_path)return;
  const response=await fetch(`${C.SUPABASE_URL}/storage/v1/object/service-documents/${d.storage_path}`,{
    method:'DELETE',
    headers:{apikey:C.SUPABASE_ANON_KEY,Authorization:`Bearer ${state.session.access_token}`}
  });
  if(!response.ok&&response.status!==404){
    const text=await response.text().catch(()=>'');
    throw Error(`Impossibile eliminare il PDF dallo Storage${text?`: ${text}`:''}`);
  }
}
window.deleteDocumentPermanently=async id=>{
  ...
  const label=documentTypeLabel(d);
  const filename=d.file_name||d.title||'Documento';
  const confirmation=prompt(
    `ELIMINAZIONE DEFINITIVA\n\n${label}\n${filename}\n\nIl PDF e il relativo record verranno eliminati definitivamente. Questa operazione non può essere annullata.\n\nScrivi ELIMINA per confermare:`,
    ''
  );
  if(confirmation!=='ELIMINA')return toast('Eliminazione annullata.');

  try{
    await deleteStorageObjectIfPresent(d);
    await rpc('delete_document_permanently',{
      p_document_id:d.id,
      p_source_kind:d.source_kind||'service'
    });
    await loadAll();

    const visible=$$('.screen:not(.hidden)')[0]?.id||'documents';
    if(visible==='documents')renderDocs();
    else if(visible==='customers')renderCustomers();
    else if(visible==='services')renderServices();
    else render(visible);

    toast(`${label} eliminato definitivamente.`);
  }catch(e){
    toast(e?.message||'Eliminazione documento non riuscita.');
  }
};

...
function receiptDataForService(id){
  ...
  if(service.status!=='chiuso')throw Error('La ricevuta è disponibile solo dopo la chiusura del servizio.');
  if(service.customer_payment_status!=='incassato')throw Error('Prima di emettere la ricevuta, registra il pagamento del cliente come incassato.');
...
async function makeServiceReceipt(id){
  if(!window.K9PdfEngine?.createReceiptPdf)throw Error('Motore ricevuta non disponibile. Aggiorna la pagina.');
  return window.K9PdfEngine.createReceiptPdf(receiptDataForService(id),state.settings||DEFAULT_SETTINGS,identityLogo(),'assets/timbro-firma.png');
}

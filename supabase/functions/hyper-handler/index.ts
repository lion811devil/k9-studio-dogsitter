...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'delete_user') {
      ...
      const linkedChecks = [
        ['servizi', 'dogsitter_services', 'employee_id'],
        ['clienti assegnati', 'customers', 'assigned_employee_id'],
        ['comunicazioni operative', 'service_communications', 'author_id'],
      ] as const;
      const linked: string[] = [];
      for (const [label, table, column] of linkedChecks) {
        const { count, error } = await admin.from(table).select('id', { count: 'exact', head: true }).eq(column, targetId);
        if (error) {
          console.error('USER_DELETE_LINK_CHECK_ERROR', { requestId, targetId, table, error: error.message });
          return reply(500, { message: `Controllo collegamenti non riuscito: ${error.message}` }, requestId);
        }
        if ((count || 0) > 0) linked.push(`${label}: ${count}`);
      }
      if (linked.length) {
        return reply(409, {

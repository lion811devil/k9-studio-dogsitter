...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!token) return reply(401, { message: 'Sessione non valida: token mancante' });
    ...
    if (userErr || !user) {
      ...
      return reply(401, { message: `Sessione non valida${userErr?.message ? `: ${userErr.message}` : ''}` });
    }
    ...
    if (callerErr || !caller || caller.active !== true || (!callerIsOwner && !callerIsVice)) {
      ...
      return reply(403, { message: 'Permesso negato: il profilo autenticato non risulta titolare o vice amministratore attivo' });
    }

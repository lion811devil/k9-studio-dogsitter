...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(action==='reset_password'){
      ...
      if(!targetId||newPassword.length<8)return reply(400,{message:'Utente e password di almeno 8 caratteri sono obbligatori'});
      const {data:target,error:targetErr}=await admin.from('profiles').select('role,is_owner,active').eq('id',targetId).single();
      if(targetErr||!target)return reply(404,{message:'Utente non trovato'});
      if(target.is_owner||target.role==='owner')return reply(403,{message:'La password del titolare non può essere modificata da questa funzione'});
      if(caller.role==='vice_admin'&&target.role!=='dipendente')return reply(403,{message:'Il Vice Amministratore può reimpostare solo password dei dipendenti'});

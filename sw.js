// K9 Studio Dogsitter 12.9.42 — Service Worker app-shell
const K9_SW_VERSION='12.9.42';
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith(caches.match(req,{ignoreSearch:true}).then(cached=>{
    const network=fetch(req,{cache:'no-store'}).then(res=>{if(res.ok){const copy=res.clone();caches.open(CACHE).then(c=>c.put(req,copy))}return res}).catch(()=>cached);
